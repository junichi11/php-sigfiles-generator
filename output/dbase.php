...
namespace {
	 ...
	 * @param array $record <p>An indexed array of data. The number of items must be equal to the number of fields in the database, otherwise <b>dbase_add_record()</b> will fail.</p> <p><b>Note</b>:</p><p>If you're using <code>dbase_get_record()</code> return value for this parameter, remember to reset the key named <i>deleted</i>.</p>
	...
	/**
	 * Creates a database
	 * <p><b>dbase_create()</b> creates a dBase database with the given definition. If the file already exists, it is not truncated. <code>dbase_pack()</code> can be called to force truncation.</p><p><b>Note</b>: When safe mode is enabled, PHP checks whether the files or directories being operated upon have the same UID (owner) as the script that is being executed.</p><p><b>Note</b>:</p><p>This function is affected by open_basedir.</p>
	 ...
	 * @return array <p>An indexed array with the record. This array will also include an associative key named <i>deleted</i> which is set to 1 if the record has been marked for deletion (see <code>dbase_delete_record()</code>).</p><p>Each field is converted to the appropriate PHP type, except:</p><ul> <li>  Dates are left as strings.  </li> <li>  DateTime values are converted to strings.  </li> <li>  Integers outside the range <b><code>PHP_INT_MIN</code></b>..<b><code>PHP_INT_MAX</code></b> are returned as strings.  </li> <li>  Before dbase 7.0.0, booleans (<i>L</i>) were converted to <i>1</i> or <i>0</i>.  </li> </ul><p>On error, <b>dbase_get_record()</b> will return <b><code>FALSE</code></b>.</p>
	 ...
	 * @param resource $dbase_identifier <p>The database link identifier, returned by <code>dbase_open()</code> or <code>dbase_create()</code>.</p>
	 * @param int $record_number <p>The index of the record between <i>1</i> and <i>dbase_numrecords($dbase_identifier)</i>.</p>
	 * @return array <p>An associative array with the record. This will also include a key named <i>deleted</i> which is set to 1 if the record has been marked for deletion (see <code>dbase_delete_record()</code>). Therefore it is not possible to retrieve the value of a field named <i>deleted</i> with this function.</p><p>Each field is converted to the appropriate PHP type, except:</p><ul> <li>  Dates are left as strings.  </li> <li>  DateTime values are converted to strings.  </li> <li>  Integers outside the range <b><code>PHP_INT_MIN</code></b>..<b><code>PHP_INT_MAX</code></b> are returned as strings.  </li> <li>  Before dbase 7.0.0, booleans (<i>L</i>) were converted to <i>1</i> or <i>0</i>.  </li> </ul><p>On error, <b>dbase_get_record_with_names()</b> will return <b><code>FALSE</code></b>.</p>
	 ...
	 * <p>Gets the number of fields (columns) in the specified database.</p><p><b>Note</b>:</p><p>Field numbers are between 0 and <i>dbase_numfields($db)-1</i>, while record numbers are between 1 and <i>dbase_numrecords($db)</i>.</p>
	 * @param resource $dbase_identifier <p>The database link identifier, returned by <code>dbase_open()</code> or <code>dbase_create()</code>.</p>
	 ...
	 * <p>Gets the number of records (rows) in the specified database.</p><p><b>Note</b>:</p><p>Record numbers are between 1 and <i>dbase_numrecords($db)</i>, while field numbers are between 0 and <i>dbase_numfields($db)-1</i>.</p>
	 * @param resource $dbase_identifier <p>The database link identifier, returned by <code>dbase_open()</code> or <code>dbase_create()</code>.</p>
	...
	/**
	 * Opens a database
	 * <p><b>dbase_open()</b> opens a dBase database with the given access mode.</p><p><b>Note</b>: When safe mode is enabled, PHP checks whether the files or directories being operated upon have the same UID (owner) as the script that is being executed.</p><p><b>Note</b>:</p><p>This function is affected by open_basedir.</p>
	 * @param string $filename <p>The name of the database. It can be a relative or absolute path to the file where dBase will store your data.</p>
	 ...
	 * @param resource $dbase_identifier <p>The database link identifier, returned by <code>dbase_open()</code> or <code>dbase_create()</code>.</p>
	 * @param array $record <p>An indexed array of data. The number of items must be equal to the number of fields in the database, otherwise <b>dbase_replace_record()</b> will fail.</p> <p><b>Note</b>:</p><p>If you're using <code>dbase_get_record()</code> return value for this parameter, remember to reset the key named <i>deleted</i>.</p>
	 * @param int $record_number <p>An integer which spans from 1 to the number of records in the database (as returned by <code>dbase_numrecords()</code>).</p>
	 * @return bool <p>Returns <b><code>TRUE</code></b> on success or <b><code>FALSE</code></b> on failure.</p>
