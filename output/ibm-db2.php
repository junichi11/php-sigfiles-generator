...
namespace {
	 ...
	 * @param string $variable_name <p>A string specifying the name of the PHP variable to bind to the parameter specified by <code>parameter-number</code>.</p>
	 * @param int $parameter_type <p>A constant specifying whether the PHP variable should be bound to the SQL parameter as an input parameter (<i>DB2_PARAM_IN</i>), an output parameter (<i>DB2_PARAM_OUT</i>), or as a parameter that accepts input and returns output (<i>DB2_PARAM_INOUT</i>). To avoid memory overhead, you can also specify <i>DB2_PARAM_FILE</i> to bind the PHP variable to the name of a file that contains large object (BLOB, CLOB, or DBCLOB) data.</p>
	 * @param int $data_type <p>A constant specifying the SQL data type that the PHP variable should be bound as: one of <i>DB2_BINARY</i>, <i>DB2_CHAR</i>, <i>DB2_DOUBLE</i>, or <i>DB2_LONG</i> .</p>
	 ...
	 * @param array $options <p>An associative array of connection options that affect the behavior of the connection, where valid array keys include:</p>  <code>autocommit</code>  <p>Passing the <i>DB2_AUTOCOMMIT_ON</i> value turns autocommit on for this connection handle.</p> <p>Passing the <i>DB2_AUTOCOMMIT_OFF</i> value turns autocommit off for this connection handle.</p>   <code>DB2_ATTR_CASE</code>  <p>Passing the <i>DB2_CASE_NATURAL</i> value specifies that column names are returned in natural case.</p> <p>Passing the <i>DB2_CASE_LOWER</i> value specifies that column names are returned in lower case.</p> <p>Passing the <i>DB2_CASE_UPPER</i> value specifies that column names are returned in upper case.</p>   <code>CURSOR</code>  <p>Passing the <i>DB2_FORWARD_ONLY</i> value specifies a forward-only cursor for a statement resource. This is the default cursor type and is supported on all database servers.</p> <p>Passing the <i>DB2_SCROLLABLE</i> value specifies a scrollable cursor for a statement resource. This mode enables random access to rows in a result set, but currently is supported only by IBM DB2 Universal Database.</p>   <p>The following new option is available in ibm_db2 version 1.7.0 and later.</p>  <code>trustedcontext</code>  <p>Passing the DB2_TRUSTED_CONTEXT_ENABLE value turns trusted context on for this connection handle. This parameter cannot be set using <code>db2_set_option()</code>.</p> <p>This key works only if the database is cataloged (even if the database is local), or if you specify the full DSN when you create the connection.</p> <p>To catalog the database, use following commands:</p> <p></p><pre>db2 catalog tcpip node loopback remote &lt;SERVERNAME&gt; server &lt;SERVICENAME&gt; db2 catalog database &lt;LOCALDBNAME&gt; as &lt;REMOTEDBNAME&gt; at node loopback db2 "update dbm cfg using svcename &lt;SERVICENAME&gt;" db2set DB2COMM=TCPIP</pre>   <p>The following new i5/OS options are available in ibm_db2 version 1.5.1 and later.</p>  <code>i5_lib</code>  <p>A character value that indicates the default library that will be used for resolving unqualified file references. This is not valid if the connection is using system naming mode.</p>   <code>i5_naming</code>  <p><i>DB2_I5_NAMING_ON</i> value turns on DB2 UDB CLI iSeries system naming mode. Files are qualified using the slash (/) delimiter. Unqualified files are resolved using the library list for the job.</p> <p><i>DB2_I5_NAMING_OFF</i> value turns off DB2 UDB CLI default naming mode, which is SQL naming. Files are qualified using the period (.) delimiter. Unqualified files are resolved using either the default library or the current user ID.</p>   <code>i5_commit</code>  <p>The <code>i5_commit</code> attribute should be set before the <b>db2_connect()</b>. If the value is changed after the connection has been established, and the connection is to a remote data source, the change does not take effect until the next successful <b>db2_connect()</b> for the connection handle.</p><p><b>Note</b>:</p><p>The php.ini setting <code>ibm_db2.i5_allow_commit</code>==0 or <i>DB2_I5_TXN_NO_COMMIT</i> is the default, but may be overridden with the <code>i5_commit</code> option.</p>  <p><i>DB2_I5_TXN_NO_COMMIT</i> - Commitment control is not used.</p> <p><i>DB2_I5_TXN_READ_UNCOMMITTED</i> - Dirty reads, nonrepeatable reads, and phantoms are possible.</p> <p><i>DB2_I5_TXN_READ_COMMITTED</i> - Dirty reads are not possible. Nonrepeatable reads, and phantoms are possible.</p> <p><i>DB2_I5_TXN_REPEATABLE_READ</i> - Dirty reads and nonrepeatable reads are not possible. Phantoms are possible.</p> <p><i>DB2_I5_TXN_SERIALIZABLE</i> - Transactions are serializable. Dirty reads, non-repeatable reads, and phantoms are not possible</p>   <code>i5_query_optimize</code>  <p><i>DB2_FIRST_IO</i> All queries are optimized with the goal of returning the first page of output as fast as possible. This goal works well when the output is controlled by a user who is most likely to cancel the query after viewing the first page of output data. Queries coded with an OPTIMIZE FOR nnn ROWS clause honor the goal specified by the clause.</p> <p><i>DB2_ALL_IO</i> All queries are optimized with the goal of running the entire query to completion in the shortest amount of elapsed time. This is a good option when the output of a query is being written to a file or report, or the interface is queuing the output data. Queries coded with an OPTIMIZE FOR nnn ROWS clause honor the goal specified by the clause. This is the default.</p>   <code>i5_dbcs_alloc</code>  <p><i>DB2_I5_DBCS_ALLOC_ON</i> value turns on DB2 6X allocation scheme for DBCS translation column size growth.</p> <p><i>DB2_I5_DBCS_ALLOC_OFF</i> value turns off DB2 6X allocation scheme for DBCS translation column size growth.</p> <p>Note: php.ini setting <code>ibm_db2.i5_dbcs_alloc</code>==0 or <i>DB2_I5_DBCS_ALLOC_OFF</i> is the default, but may be overridden with the <code>i5_dbcs_alloc</code> option.</p>   <code>i5_date_fmt</code>  <p><i>DB2_I5_FMT_ISO</i> - The International Organization for Standardization (ISO) date format yyyy-mm-dd is used. This is the default.</p> <p><i>DB2_I5_FMT_USA</i> - The United States date format mm/dd/yyyy is used.</p> <p><i>DB2_I5_FMT_EUR</i> - The European date format dd.mm.yyyy is used.</p> <p><i>DB2_I5_FMT_JIS</i> - The Japanese Industrial Standard date format yyyy-mm-dd is used.</p> <p><i>DB2_I5_FMT_MDY</i> - The date format mm/dd/yyyy is used.</p> <p><i>DB2_I5_FMT_DMY</i> - The date format dd/mm/yyyy is used.</p> <p><i>DB2_I5_FMT_YMD</i> - The date format yy/mm/dd is used.</p> <p><i>DB2_I5_FMT_JUL</i> - The Julian date format yy/ddd is used.</p> <p><i>DB2_I5_FMT_JOB</i> - The job default is used.</p>   <code>i5_date_sep</code>  <p><i>DB2_I5_SEP_SLASH</i> - A slash ( / ) is used as the date separator. This is the default.</p> <p><i>DB2_I5_SEP_DASH</i> - A dash ( - ) is used as the date separator.</p> <p><i>DB2_I5_SEP_PERIOD</i> - A period ( . ) is used as the date separator.</p> <p><i>DB2_I5_SEP_COMMA</i> - A comma ( , ) is used as the date separator.</p> <p><i>DB2_I5_SEP_BLANK</i> - A blank is used as the date separator.</p> <p><i>DB2_I5_SEP_JOB</i> - The job default is used</p>   <code>i5_time_fmt</code>  <p><i>DB2_I5_FMT_ISO</i> - The International Organization for Standardization (ISO) time format hh.mm.ss is used. This is the default.</p> <p><i>DB2_I5_FMT_USA</i> - The United States time format hh:mmxx is used, where xx is AM or PM.</p> <p><i>DB2_I5_FMT_EUR</i> - The European time format hh.mm.ss is used.</p> <p><i>DB2_I5_FMT_JIS</i> - The Japanese Industrial Standard time format hh:mm:ss is used.</p> <p><i>DB2_I5_FMT_HMS</i> - The hh:mm:ss format is used.</p>   <code>i5_time_sep</code>  <p><i>DB2_I5_SEP_COLON</i> - A colon ( : ) is used as the time separator. This is the default.</p> <p><i>DB2_I5_SEP_PERIOD</i> - A period ( . ) is used as the time separator.</p> <p><i>DB2_I5_SEP_COMMA</i> - A comma ( , ) is used as the time separator.</p> <p><i>DB2_I5_SEP_BLANK</i> - A blank is used as the time separator.</p> <p><i>DB2_I5_SEP_JOB</i> - The job default is used.</p>   <code>i5_decimal_sep</code>  <p><i>DB2_I5_SEP_PERIOD</i> - A period ( . ) is used as the decimal separator. This is the default.</p> <p><i>DB2_I5_SEP_COMMA</i> - A comma ( , ) is used as the decimal separator.</p> <p><i>DB2_I5_SEP_JOB</i> - The job default is used.</p>   <p>The following new i5/OS option is available in ibm_db2 version 1.8.0 and later.</p>  <code>i5_libl</code>  <p>A character value that indicates the library list that will be used for resolving unqualified file references. Specify the library list elements separated by blanks 'i5_libl'=&gt;"MYLIB YOURLIB ANYLIB".</p> <p><b>Note</b>:</p><p><code>i5_libl</code> calls qsys2/qcmdexc('cmd',cmdlen), which is only available in i5/OS V5R4 and later.</p>
	 ...
	 * @return int <p>Returns either <i>DB2_FORWARD_ONLY</i> if the statement resource uses a forward-only cursor or <i>DB2_SCROLLABLE</i> if the statement resource uses a scrollable cursor.</p>
	 ...
	 * @param string $string_literal <p>The string that contains special characters that need to be modified. Characters that are prepended with a backslash are <i>\x00</i>, <i>\n</i>, <i>\r</i>, <i>\</i>, <i>'</i>, <i>"</i> and <i>\x1a</i>.</p>
	 ...
	 * @param resource $stmt <p>A valid <i>stmt</i> resource containing a result set.</p>
	 ...
	 * @param resource $stmt <p>A valid <i>stmt</i> resource.</p>
	 ...
	 * @param string $option <p>A valid statement or connection options. The following new options are available as of ibm_db2 version 1.6.0. They provide useful tracking information that can be set during execution with <b>db2_get_option()</b>.</p><p><b>Note</b>:</p><p>Prior versions of ibm_db2 do not support these new options.</p> <p>When the value in each option is being set, some servers might not handle the entire length provided and might truncate the value.</p> <p>To ensure that the data specified in each option is converted correctly when transmitted to a host system, use only the characters A through Z, 0 through 9, and the underscore (_) or period (.).</p>    <code>userid</code>  <p><i>SQL_ATTR_INFO_USERID</i> - A pointer to a null-terminated character string used to identify the client user ID sent to the host database server when using DB2 Connect.</p><p><b>Note</b>:</p><p>DB2 for z/OS and OS/390 servers support up to a length of 16 characters. This user-id is not to be confused with the authentication user-id, it is for identification purposes only and is not used for any authorization.</p>    <code>acctstr</code>  <p><i>SQL_ATTR_INFO_ACCTSTR</i> - A pointer to a null-terminated character string used to identify the client accounting string sent to the host database server when using DB2 Connect.</p><p><b>Note</b>:</p><p>DB2 for z/OS and OS/390 servers support up to a length of 200 characters.</p>    <code>applname</code>  <p><i>SQL_ATTR_INFO_APPLNAME</i> - A pointer to a null-terminated character string used to identify the client application name sent to the host database server when using DB2 Connect.</p><p><b>Note</b>:</p><p>DB2 for z/OS and OS/390 servers support up to a length of 32 characters.</p>    <code>wrkstnname</code>  <p><i>SQL_ATTR_INFO_WRKSTNNAME</i> - A pointer to a null-terminated character string used to identify the client workstation name sent to the host database server when using DB2 Connect.</p><p><b>Note</b>:</p><p>DB2 for z/OS and OS/390 servers support up to a length of 18 characters.</p>
	 * @return string <p>Returns the current setting of the connection attribute provided on success or <b><code>FALSE</code></b> on failure.</p>
	 ...
	 * @param resource $stmt <p>A valid <i>stmt</i> resource containing LOB data.</p>
	 * @param int $colnum <p>A valid column number in the result set of the <i>stmt</i> resource.</p>
	 * @param int $length <p>The size of the LOB data to be retrieved from the <i>stmt</i> resource.</p>
	 ...
	 * @return bool <p>Returns <b><code>TRUE</code></b> on success or <b><code>FALSE</code></b> on failure.</p>
	 * @link http://php.net/manual/en/function.db2-pclose.php
	 * @see db2_close(), db2_pconnect()
	 ...
	 * @param resource $resource <p>A valid statement resource as returned from <code>db2_prepare()</code> or a valid connection resource as returned from <code>db2_connect()</code> or <code>db2_pconnect()</code>.</p>
	 * @param array $options <p>An associative array containing valid statement or connection options. This parameter can be used to change autocommit values, cursor types (scrollable or forward), and to specify the case of the column names (lower, upper, or natural) that will appear in a result set.</p>  <code>autocommit</code>  <p>Passing <i>DB2_AUTOCOMMIT_ON</i> turns autocommit on for the specified connection resource.</p> <p>Passing <i>DB2_AUTOCOMMIT_OFF</i> turns autocommit off for the specified connection resource.</p>   <code>cursor</code>  <p>Passing <i>DB2_FORWARD_ONLY</i> specifies a forward-only cursor for a statement resource. This is the default cursor type, and is supported by all database servers.</p> <p>Passing <i>DB2_SCROLLABLE</i> specifies a scrollable cursor for a statement resource. Scrollable cursors enable result set rows to be accessed in non-sequential order, but are only supported by IBM DB2 Universal Database databases.</p>   <code>binmode</code>  <p>Passing <i>DB2_BINARY</i> specifies that binary data will be returned as is. This is the default mode. This is the equivalent of setting <i>ibm_db2.binmode=1</i> in php.ini.</p> <p>Passing <i>DB2_CONVERT</i> specifies that binary data will be converted to hexadecimal encoding, and will be returned as such. This is the equivalent of setting <i>ibm_db2.binmode=2</i> in php.ini.</p> <p>Passing <i>DB2_PASSTHRU</i> specifies that binary data will be converted to <b><code>NULL</code></b>. This is the equivalent of setting <i>ibm_db2.binmode=3</i> in php.ini.</p>   <code>db2_attr_case</code>  <p>Passing <i>DB2_CASE_LOWER</i> specifies that column names of the result set are returned in lower case.</p> <p>Passing <i>DB2_CASE_UPPER</i> specifies that column names of the result set are returned in upper case.</p> <p>Passing <i>DB2_CASE_NATURAL</i> specifies that column names of the result set are returned in natural case.</p>   <code>deferred_prepare</code>  <p>Passing <i>DB2_DEFERRED_PREPARE_ON</i> turns deferred prepare on for the specified statement resource.</p> <p>Passing <i>DB2_DEFERRED_PREPARE_OFF</i> turns deferred prepare off for the specified statement resource.</p>   <p>The following new i5/OS options are available in ibm_db2 version 1.5.1 and later. These options apply only when running PHP and ibm_db2 natively on i5 systems.</p>  <code>i5_fetch_only</code>  <p><i>DB2_I5_FETCH_ON</i> - Cursors are read-only and cannot be used for positioned updates or deletes. This is the default unless <i>SQL_ATTR_FOR_FETCH_ONLY</i> environment has been set to <i>SQL_FALSE</i>.</p> <p><i>DB2_I5_FETCH_OFF</i> - Cursors can be used for positioned updates and deletes.</p>   <p>The following new option is available in ibm_db2 version 1.8.0 and later.</p>  <code>rowcount</code>  <p><i>DB2_ROWCOUNT_PREFETCH_ON</i> - Client can request the full row count prior to fetching, which means that <code>db2_num_rows()</code> returns the number of rows selected even when a <i>ROLLFORWARD_ONLY</i> cursor is used.</p> <p><i>DB2_ROWCOUNT_PREFETCH_OFF</i> - Client cannot request the full row count prior to fetching.</p>   <p>The following new options are available in ibm_db2 version 1.7.0 and later.</p>  <code>trusted_user</code>  <p>To switch the user to a trusted user, pass the User ID (String) of the trusted user as the value of this key. This option can be set on a connection resource only. To use this option, trusted context must be enabled on the connection resource.</p>   <code>trusted_password</code>  <p>The password (String) that corresponds to the user specified by the trusted_user key.</p>   <p>The following new options are available in ibm_db2 version 1.6.0 and later. These options provide useful tracking information that can be accessed during execution with <code>db2_get_option()</code>.</p><p><b>Note</b>:</p><p>When the value in each option is being set, some servers might not handle the entire length provided and might truncate the value.</p> <p>To ensure that the data specified in each option is converted correctly when transmitted to a host system, use only the characters A through Z, 0 through 9, and the underscore (_) or period (.).</p>    <code>userid</code>  <p><i>SQL_ATTR_INFO_USERID</i> - A pointer to a null-terminated character string used to identify the client user ID sent to the host database server when using DB2 Connect.</p><p><b>Note</b>:</p><p>DB2 for z/OS and OS/390 servers support up to a length of 16 characters. This user-id is not to be confused with the authentication user-id, it is for identification purposes only and is not used for any authorization.</p>    <code>acctstr</code>  <p><i>SQL_ATTR_INFO_ACCTSTR</i> - A pointer to a null-terminated character string used to identify the client accounting string sent to the host database server when using DB2 Connect.</p><p><b>Note</b>:</p><p>DB2 for z/OS and OS/390 servers support up to a length of 200 characters.</p>    <code>applname</code>  <p><i>SQL_ATTR_INFO_APPLNAME</i> - A pointer to a null-terminated character string used to identify the client application name sent to the host database server when using DB2 Connect.</p><p><b>Note</b>:</p><p>DB2 for z/OS and OS/390 servers support up to a length of 32 characters.</p>    <code>wrkstnname</code>  <p><i>SQL_ATTR_INFO_WRKSTNNAME</i> - A pointer to a null-terminated character string used to identify the client workstation name sent to the host database server when using DB2 Connect.</p><p><b>Note</b>:</p><p>DB2 for z/OS and OS/390 servers support up to a length of 18 characters.</p>
	 * @param int $type <p>An integer value that specifies the type of resource that was passed into the function. The type of resource and this value must correspond.</p> <p>Passing <i>1</i> as the value specifies that a connection resource has been passed into the function.</p> <p>Passing any integer not equal to <i>1</i> as the value specifies that a statement resource has been passed into the function.</p>
	 * @return bool <p>Returns <b><code>TRUE</code></b> on success or <b><code>FALSE</code></b> on failure.</p>
	 ...
	 * @param resource $connection <p>A valid connection to an IBM DB2, Cloudscape, or Apache Derby database.</p>
	 * @param string $qualifier <p>A qualifier for DB2 databases running on OS/390 or z/OS servers. For other databases, pass <b><code>NULL</code></b> or an empty string.</p>
	 * @param string $schema <p>The schema which contains the tables. This parameter accepts a search pattern containing <i>_</i> and <i>%</i> as wildcards.</p>
	 * @param string $table_name <p>The name of the table. This parameter accepts a search pattern containing <i>_</i> and <i>%</i> as wildcards.</p>
