...
namespace {
	 ...
	 * @see fwrite(), fopen(), fsockopen(), popen(), fgets(), fgetss(), fscanf(), file(), fpassthru(), fseek(), ftell(), rewind(), unpack()
	 * @since PHP 4, PHP 5, PHP 7, PHP 8
	 */
	 ...
	 * @param string $data <p>The string that is to be written.</p>
	 * @param ?int $length <p>If <code>length</code> is an <code>int</code>, writing will stop after <code>length</code> bytes have been written or the end of <code>data</code> is reached, whichever comes first.</p>
	 * @return int|false <p><b>fwrite()</b> returns the number of bytes written, or <b><code>false</code></b> on failure.</p>
	 ...
	 * <p>Returns <b><code>true</code></b> if the <code>filename</code> exists and is writable. The filename argument may be a directory name allowing you to check if a directory is writable.</p><p>Keep in mind that PHP may be accessing the file as the user id that the web server runs as (often 'nobody').</p>
	 * @param string $filename <p>The filename being checked.</p>
	 ...
	 * @param string $directory <p>The directory path.</p><b>Tip</b><p>A URL can be used as a filename with this function if the fopen wrappers have been enabled. See <code>fopen()</code> for more details on how to specify the filename. See the Supported Protocols and Wrappers for links to information about what abilities the various wrappers have, notes on their usage, and information on any predefined variables they may provide.</p>
	 ...
	 * @param bool $recursive <p>If <b><code>true</code></b>, then any parent directories to the <code>directory</code> specified will also be created, with the same permissions.</p>
	 * @param ?resource $context <p>A context stream <code>resource</code>.</p>
	 * @return bool <p>Returns <b><code>true</code></b> on success or <b><code>false</code></b> on failure.</p><p><b>Note</b>:</p><p>If the directory to be created already exists, that is considered an error and <b><code>false</code></b> will still be returned. Use <code>is_dir()</code> or <code>file_exists()</code> to check if the directory already exists before trying to create it.</p>
	 * @link https://php.net/manual/en/function.mkdir.php
	 * @see is_dir(), rmdir(), umask()
	 * @since PHP 4, PHP 5, PHP 7, PHP 8
	 */
	 ...
	 * @param string $prefix <p>The prefix of the generated temporary filename.</p> <p><b>Note</b>:  Only the first 63 characters of the prefix are used, the rest are ignored. Windows uses only the first three characters of the prefix. </p>
	 ...
	 * <p>Creates a temporary file with a unique name in read-write-binary (w+b) mode and returns a file handle.</p><p>The file is automatically removed when closed (for example, by calling <code>fclose()</code>, or when there are no remaining references to the file handle returned by <b>tmpfile()</b>), or when the script ends.</p><p>If the script terminates unexpectedly, the temporary file may not be deleted.</p>
	 ...
	 * @param ?int $atime <p>If not <b><code>null</code></b>, the access time of the given filename is set to the value of <code>atime</code>. Otherwise, it is set to the value passed to the <code>mtime</code> parameter. If both are <b><code>null</code></b>, the current system time is used.</p>
	 * @return bool <p>Returns <b><code>true</code></b> on success or <b><code>false</code></b> on failure.</p>
