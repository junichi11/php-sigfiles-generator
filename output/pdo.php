...
namespace {
	...
	class PDO {
		...
		/**
		 * @var int Force column names to a specific case specified by the <i>PDO::CASE_&#42;</i> constants.
		 * @link http://php.net/manual/en/pdo.constants.php
		 */
		const ATTR_CASE = 8;
		...
		/**
		 * @var int Returns the name of the driver.  <p></p><p><b>Example #1 using <b><code>PDO::ATTR_DRIVER_NAME</code></b></b></p>  <code> &lt;&#63;php<br>if&nbsp;($db-&gt;getAttribute(PDO::ATTR_DRIVER_NAME)&nbsp;==&nbsp;'mysql')&nbsp;{<br>&nbsp;&nbsp;echo&nbsp;"Running&nbsp;on&nbsp;mysql;&nbsp;doing&nbsp;something&nbsp;mysql&nbsp;specific&nbsp;here\n";<br>}<br>&#63;&gt;  </code>
		 * @link http://php.net/manual/en/pdo.constants.php
		 */
		...
		/**
		 * @var int Request a persistent connection, rather than creating a new connection. See Connections and Connection management for more information on this attribute.
		 * @link http://php.net/manual/en/pdo.constants.php
		 */
		...
		/**
		 * Set an attribute
		 * <p>Sets an attribute on the database handle. Some of the available generic attributes are listed below; some drivers may make use of additional driver specific attributes.</p><p><i>PDO::ATTR_CASE</i>: Force column names to a specific case.</p><p><i>PDO::CASE_LOWER</i>: Force column names to lower case.</p><p><i>PDO::CASE_NATURAL</i>: Leave column names as returned by the database driver.</p><p><i>PDO::CASE_UPPER</i>: Force column names to upper case.</p><p><i>PDO::ATTR_ERRMODE</i>: Error reporting.</p><p><i>PDO::ERRMODE_SILENT</i>: Just set error codes.</p><p><i>PDO::ERRMODE_WARNING</i>: Raise E_WARNING.</p><p><i>PDO::ERRMODE_EXCEPTION</i>: Throw exceptions.</p><p><i>PDO::ATTR_ORACLE_NULLS</i> (available with all drivers, not just Oracle): Conversion of NULL and empty strings.</p><p><i>PDO::NULL_NATURAL</i>: No conversion.</p><p><i>PDO::NULL_EMPTY_STRING</i>: Empty string is converted to <b><code>NULL</code></b>.</p><p><i>PDO::NULL_TO_STRING</i>: NULL is converted to an empty string.</p><p><i>PDO::ATTR_STRINGIFY_FETCHES</i>: Convert numeric values to strings when fetching. Requires <code>bool</code>.</p><p><i>PDO::ATTR_STATEMENT_CLASS</i>: Set user-supplied statement class derived from PDOStatement. Cannot be used with persistent PDO instances. Requires <i>array(string classname, array(mixed constructor_args))</i>.</p><p><i>PDO::ATTR_TIMEOUT</i>: Specifies the timeout duration in seconds. Not all drivers support this option, and its meaning may differ from driver to driver. For example, sqlite will wait for up to this time value before giving up on obtaining an writable lock, but other drivers may interpret this as a connect or a read timeout interval. Requires <code>int</code>.</p><p><i>PDO::ATTR_AUTOCOMMIT</i> (available in OCI, Firebird and MySQL): Whether to autocommit every single statement.</p><p><i>PDO::ATTR_EMULATE_PREPARES</i> Enables or disables emulation of prepared statements. Some drivers do not support native prepared statements or have limited support for them. Use this setting to force PDO to either always emulate prepared statements (if <b><code>TRUE</code></b> and emulated prepares are supported by the driver), or to try to use native prepared statements (if <b><code>FALSE</code></b>). It will always fall back to emulating the prepared statement if the driver cannot successfully prepare the current query. Requires <code>bool</code>.</p><p><i>PDO::MYSQL_ATTR_USE_BUFFERED_QUERY</i> (available in MySQL): Use buffered queries.</p><p><i>PDO::ATTR_DEFAULT_FETCH_MODE</i>: Set default fetch mode. Description of modes is available in <code>PDOStatement::fetch()</code> documentation.</p>
		 * @param int $attribute
		 * @param mixed $value
		 * @return bool <p>Returns <b><code>TRUE</code></b> on success or <b><code>FALSE</code></b> on failure.</p>
	...
	class PDOException extends \RuntimeException {
		 ...
		 * @link http://php.net/manual/en/class.pdoexception.php#pdoexception.props.errorinfo
		 */
		public $errorInfo;
	...
	class PDOStatement implements \Traversable {
		 ...
		 * @link http://php.net/manual/en/class.pdostatement.php#pdostatement.props.querystring
		 */
		public $queryString;
		 ...
		 * @param int $type <p>Data type of the parameter, specified by the <i>PDO::PARAM_&#42;</i> constants.</p>
		 ...
		 * @param int $data_type <p>Explicit data type for the parameter using the <i>PDO::PARAM_&#42;</i> constants. To return an INOUT parameter from a stored procedure, use the bitwise OR operator to set the PDO::PARAM_INPUT_OUTPUT bits for the <code>data_type</code> parameter.</p>
		 ...
		 * @param int $data_type <p>Explicit data type for the parameter using the <i>PDO::PARAM_&#42;</i> constants.</p>
		 * @return bool <p>Returns <b><code>TRUE</code></b> on success or <b><code>FALSE</code></b> on failure.</p>
		 ...
		 * @return int <p>Returns the number of columns in the result set represented by the PDOStatement object, even if the result set is empty. If there is no result set, <b>PDOStatement::columnCount()</b> returns <i>0</i>.</p>
		 ...
		 * <p>Dumps the information contained by a prepared statement directly on the output. It will provide the <i>SQL</i> query in use, the number of parameters used (<i>Params</i>), the list of parameters with their key name or position, their name, their position in the query (if this is supported by the PDO driver, otherwise, it will be -1), type (<i>param_type</i>) as an integer, and a boolean value <i>is_param</i>.</p><p>This is a debug function, which dumps the data directly to the normal output.</p><p>As with anything that outputs its result directly to the browser, the output-control functions can be used to capture the output of this function, and save it in a <code>string</code> (for example).</p><p>This will only dump the parameters in the statement at the moment of the dump. Extra parameters are not stored in the statement, and not displayed.</p>
		 * @return void <p>No value is returned.</p>
		 ...
		 * @param int $fetch_style <p>Controls how the next row will be returned to the caller. This value must be one of the <i>PDO::FETCH_&#42;</i> constants, defaulting to value of <i>PDO::ATTR_DEFAULT_FETCH_MODE</i> (which defaults to <i>PDO::FETCH_BOTH</i>).</p><ul> <li><p><i>PDO::FETCH_ASSOC</i>: returns an array indexed by column name as returned in your result set</p></li> <li><p><i>PDO::FETCH_BOTH</i> (default): returns an array indexed by both column name and 0-indexed column number as returned in your result set</p></li> <li><p><i>PDO::FETCH_BOUND</i>: returns <b><code>TRUE</code></b> and assigns the values of the columns in your result set to the PHP variables to which they were bound with the <code>PDOStatement::bindColumn()</code> method</p></li> <li><p><i>PDO::FETCH_CLASS</i>: returns a new instance of the requested class, mapping the columns of the result set to named properties in the class, and calling the constructor afterwards, unless <i>PDO::FETCH_PROPS_LATE</i> is also given. If <code>fetch_style</code> includes PDO::FETCH_CLASSTYPE (e.g. <i>PDO::FETCH_CLASS | PDO::FETCH_CLASSTYPE</i>) then the name of the class is determined from a value of the first column.</p></li> <li><p><i>PDO::FETCH_INTO</i>: updates an existing instance of the requested class, mapping the columns of the result set to named properties in the class</p></li> <li><p><i>PDO::FETCH_LAZY</i>: combines <i>PDO::FETCH_BOTH</i> and <i>PDO::FETCH_OBJ</i>, creating the object variable names as they are accessed</p></li> <li><p><i>PDO::FETCH_NAMED</i>: returns an array with the same form as <i>PDO::FETCH_ASSOC</i>, except that if there are multiple columns with the same name, the value referred to by that key will be an array of all the values in the row that had that column name</p></li> <li><p><i>PDO::FETCH_NUM</i>: returns an array indexed by column number as returned in your result set, starting at column 0</p></li> <li><p><i>PDO::FETCH_OBJ</i>: returns an anonymous object with property names that correspond to the column names returned in your result set</p></li> <li><p><i>PDO::FETCH_PROPS_LATE</i>: when used with <i>PDO::FETCH_CLASS</i>, the constructor of the class is called before the properties are assigned from the respective column values.</p></li> </ul>
		 * @param int $cursor_orientation <p>For a PDOStatement object representing a scrollable cursor, this value determines which row will be returned to the caller. This value must be one of the <i>PDO::FETCH_ORI_&#42;</i> constants, defaulting to <i>PDO::FETCH_ORI_NEXT</i>. To request a scrollable cursor for your PDOStatement object, you must set the <i>PDO::ATTR_CURSOR</i> attribute to <i>PDO::CURSOR_SCROLL</i> when you prepare the SQL statement with <code>PDO::prepare()</code>.</p>
		 ...
		 * @return mixed <p>Returns an instance of the required class with property names that correspond to the column names or <b><code>FALSE</code></b> on failure.</p>
		 ...
		 * <p>Gets an attribute of the statement. Currently, no generic attributes exist but only driver specific:</p><p><i>PDO::ATTR_CURSOR_NAME</i> (Firebird and ODBC specific): Get the name of cursor for <i>UPDATE ... WHERE CURRENT OF</i>.</p>
		 * @param int $attribute
		 ...
		 * @return array <p>Returns an associative array containing the following values representing the metadata for a single column:</p> <b>Column metadata</b>     Name Value     <i>native_type</i> The PHP native type used to represent the column value.   <i>driver:decl_type</i> The SQL type used to represent the column value in the database. If the column in the result set is the result of a function, this value is not returned by <b>PDOStatement::getColumnMeta()</b>.    <i>flags</i> Any flags set for this column.   <i>name</i> The name of this column as returned by the database.   <i>table</i> The name of this column's table as returned by the database.   <i>len</i> The length of this column. Normally <i>-1</i> for types other than floating point decimals.   <i>precision</i> The numeric precision of this column. Normally <i>0</i> for types other than floating point decimals.   <i>pdo_type</i> The type of this column as represented by the <i>PDO::PARAM_&#42;</i> constants.   <p>Returns <b><code>FALSE</code></b> if the requested column does not exist in the result set, or if no result set exists.</p>
		 ...
		 * <p><b>PDOStatement::rowCount()</b> returns the number of rows affected by the last DELETE, INSERT, or UPDATE statement executed by the corresponding <i>PDOStatement</i> object.</p><p>If the last SQL statement executed by the associated <i>PDOStatement</i> was a SELECT statement, some databases may return the number of rows returned by that statement. However, this behaviour is not guaranteed for all databases and should not be relied on for portable applications.</p>
		 ...
		 * <p>Sets an attribute on the statement. Currently, no generic attributes are set but only driver specific:</p><p><i>PDO::ATTR_CURSOR_NAME</i> (Firebird and ODBC specific): Set the name of cursor for <i>UPDATE ... WHERE CURRENT OF</i>.</p>
		 * @param int $attribute
		 * @param mixed $value
		 * @return bool <p>Returns <b><code>TRUE</code></b> on success or <b><code>FALSE</code></b> on failure.</p>
