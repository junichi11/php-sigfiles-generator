...
namespace {
	 ...
	 * @param string $hostname <p><code>hostname</code> should be a valid DNS hostname such as "<i>www.example.com</i>". Reverse lookups can be generated using <i>in-addr.arpa</i> notation, but <code>gethostbyaddr()</code> is more suitable for the majority of reverse lookups.</p> <p><b>Note</b>:</p><p>Per DNS standards, email addresses are given in <i>user.host</i> format (for example: <i>hostmaster.example.com</i> as opposed to <i>hostmaster@example.com</i>), be sure to check this value and modify if necessary before using it with a functions such as <code>mail()</code>.</p>
	 * @param int $type <p>By default, <b>dns_get_record()</b> will search for any resource records associated with <code>hostname</code>. To limit the query, specify the optional <code>type</code> parameter. May be any one of the following: <b><code>DNS_A</code></b>, <b><code>DNS_CNAME</code></b>, <b><code>DNS_HINFO</code></b>, <b><code>DNS_CAA</code></b>, <b><code>DNS_MX</code></b>, <b><code>DNS_NS</code></b>, <b><code>DNS_PTR</code></b>, <b><code>DNS_SOA</code></b>, <b><code>DNS_TXT</code></b>, <b><code>DNS_AAAA</code></b>, <b><code>DNS_SRV</code></b>, <b><code>DNS_NAPTR</code></b>, <b><code>DNS_A6</code></b>, <b><code>DNS_ALL</code></b> or <b><code>DNS_ANY</code></b>.</p> <p><b>Note</b>:</p><p>Because of eccentricities in the performance of libresolv between platforms, <b><code>DNS_ANY</code></b> will not always return every record, the slower <b><code>DNS_ALL</code></b> will collect all records more reliably.</p>
	 ...
	 * @param bool $raw <p>In case of raw mode, we query only the requested type instead of looping type by type before going with the additional info stuff.</p>
	 * @return array <p>This function returns an array of associative arrays, or <b><code>FALSE</code></b> on failure. Each associative array contains <i>at minimum</i> the following keys:</p> <b>Basic DNS attributes</b>   Attribute Meaning     host  The record in the DNS namespace to which the rest of the associated data refers.    class  <b>dns_get_record()</b> only returns Internet class records and as such this parameter will always return <i>IN</i>.    type  String containing the record type. Additional attributes will also be contained in the resulting array dependant on the value of type. See table below.    ttl  <i>"Time To Live"</i> remaining for this record. This will <i>not</i> equal the record's original ttl, but will rather equal the original ttl minus whatever length of time has passed since the authoritative name server was queried.     <b>Other keys in associative arrays dependant on 'type'</b>   Type Extra Columns     <i>A</i>  <i>ip</i>: An IPv4 addresses in dotted decimal notation.    <i>MX</i>  <i>pri</i>: Priority of mail exchanger. Lower numbers indicate greater priority. <i>target</i>: FQDN of the mail exchanger. See also <code>dns_get_mx()</code>.    <i>CNAME</i>  <i>target</i>: FQDN of location in DNS namespace to which the record is aliased.    <i>NS</i>  <i>target</i>: FQDN of the name server which is authoritative for this hostname.    <i>PTR</i>  <i>target</i>: Location within the DNS namespace to which this record points.    <i>TXT</i>  <i>txt</i>: Arbitrary string data associated with this record.    <i>HINFO</i>  <i>cpu</i>: IANA number designating the CPU of the machine referenced by this record. <i>os</i>: IANA number designating the Operating System on the machine referenced by this record. See IANA's <i>Operating System Names</i> for the meaning of these values.    <i>CAA</i>  <i>flags</i>: A one-byte bitfield; currently only bit 0 is defined, meaning 'critical'; other bits are reserved and should be ignored. <i>tag</i>: The CAA tag name (alphanumeric ASCII string). <i>value</i>: The CAA tag value (binary string, may use subformats). For additional information see: RFC 6844    <i>SOA</i>  <i>mname</i>: FQDN of the machine from which the resource records originated. <i>rname</i>: Email address of the administrative contact for this domain. <i>serial</i>: Serial # of this revision of the requested domain. <i>refresh</i>: Refresh interval (seconds) secondary name servers should use when updating remote copies of this domain. <i>retry</i>: Length of time (seconds) to wait after a failed refresh before making a second attempt. <i>expire</i>: Maximum length of time (seconds) a secondary DNS server should retain remote copies of the zone data without a successful refresh before discarding. <i>minimum-ttl</i>: Minimum length of time (seconds) a client can continue to use a DNS resolution before it should request a new resolution from the server. Can be overridden by individual resource records.    <i>AAAA</i>  <i>ipv6</i>: IPv6 address    <i>A6</i>(PHP &gt;= 5.1.0)  <i>masklen</i>: Length (in bits) to inherit from the target specified by <code>chain</code>. <i>ipv6</i>: Address for this specific record to merge with <code>chain</code>. <i>chain</i>: Parent record to merge with <code>ipv6</code> data.    <i>SRV</i>  <i>pri</i>: (Priority) lowest priorities should be used first. <i>weight</i>: Ranking to weight which of commonly prioritized <code>targets</code> should be chosen at random. <i>target</i> and <i>port</i>: hostname and port where the requested service can be found. For additional information see: RFC 2782    <i>NAPTR</i>  <i>order</i> and <i>pref</i>: Equivalent to <code>pri</code> and <code>weight</code> above. <i>flags</i>, <i>services</i>, <i>regex</i>, and <i>replacement</i>: Parameters as defined by RFC 2915.
	 ...
	 * @param string $hostname <p>If OpenSSL support is installed, you may prefix the <code>hostname</code> with either <i>ssl://</i> or <i>tls://</i> to use an SSL or TLS client connection over TCP/IP to connect to the remote host.</p>
	 * @param int $port <p>The port number. This can be omitted and skipped with <i>-1</i> for transports that do not use ports, such as <i>unix://</i>.</p>
	 * @param int $errno <p>If provided, holds the system level error number that occurred in the system-level <i>connect()</i> call.</p> <p>If the value returned in <code>errno</code> is <i>0</i> and the function returned <b><code>FALSE</code></b>, it is an indication that the error occurred before the <i>connect()</i> call. This is most likely due to a problem initializing the socket.</p>
	 ...
	 * @return int <p>Returns the protocol number, or <b><code>FALSE</code></b> on failure.</p>
	 ...
	 * @return string <p>Returns the protocol name as a string, or <b><code>FALSE</code></b> on failure.</p>
	 ...
	 * @param string $protocol <p><code>protocol</code> is either <i>"tcp"</i> or <i>"udp"</i> (in lowercase).</p>
	 ...
	 * @param string $header <p>The header string.</p> <p>There are two special-case header calls. The first is a header that starts with the string "<i>HTTP/</i>" (case is not significant), which will be used to figure out the HTTP status code to send. For example, if you have configured Apache to use a PHP script to handle requests for missing files (using the <i>ErrorDocument</i> directive), you may want to make sure that your script generates the proper status code.</p> <p></p>  <code> &lt;&#63;php<br>header("HTTP/1.0&nbsp;404&nbsp;Not&nbsp;Found");<br>&#63;&gt;  </code>   <p>The second special case is the "Location:" header. Not only does it send this header back to the browser, but it also returns a <i>REDIRECT</i> (302) status code to the browser unless the <i>201</i> or a <i>3xx</i> status code has already been set.</p> <p></p>  <code> &lt;&#63;php<br>header("Location:&nbsp;http://www.example.com/");&nbsp;/&#42;&nbsp;Redirect&nbsp;browser&nbsp;&#42;/<br><br>/&#42;&nbsp;Make&nbsp;sure&nbsp;that&nbsp;code&nbsp;below&nbsp;does&nbsp;not&nbsp;get&nbsp;executed&nbsp;when&nbsp;we&nbsp;redirect.&nbsp;&#42;/<br>exit;<br>&#63;&gt;  </code>
	 ...
	 * @return mixed <p>If <code>response_code</code> is provided, then the previous status code will be returned. If <code>response_code</code> is not provided, then the current status code will be returned. Both of these values will default to a <i>200</i> status code if used in a web server environment.</p><p><b><code>FALSE</code></b> will be returned if <code>response_code</code> is not provided and it is not invoked in a web server environment (such as from a CLI application). <b><code>TRUE</code></b> will be returned if <code>response_code</code> is provided and it is not invoked in a web server environment (but only when no previous response status has been set).</p>
	 ...
	 * @return string <p>Returns a string representation of the address or <b><code>FALSE</code></b> on failure.</p>
	 ...
	 * @return string <p>Returns the <i>in_addr</i> representation of the given <code>address</code>, or <b><code>FALSE</code></b> if a syntactically invalid <code>address</code> is given (for example, an IPv4 address without dots or an IPv6 address without colons).</p>
	 ...
	 * @param int $option <p>The <code>option</code> argument is used to indicate what logging options will be used when generating a log message.</p> <b><b>openlog()</b> Options</b>   Constant Description     <b><code>LOG_CONS</code></b>  if there is an error while sending data to the system logger, write directly to the system console    <b><code>LOG_NDELAY</code></b>  open the connection to the logger immediately    <b><code>LOG_ODELAY</code></b>  (default) delay opening the connection until the first message is logged    <b><code>LOG_PERROR</code></b> print log message also to standard error   <b><code>LOG_PID</code></b> include PID with each message    You can use one or more of this options. When using multiple options you need to <i>OR</i> them, i.e. to open the connection immediately, write to the console and include the PID in each message, you will use: <i>LOG_CONS | LOG_NDELAY | LOG_PID</i>
	...
	/**
	 * Send a cookie
	 * <p><b>setcookie()</b> defines a cookie to be sent along with the rest of the HTTP headers. Like other headers, cookies must be sent <i>before</i> any output from your script (this is a protocol restriction). This requires that you place calls to this function prior to any output, including <i>&lt;html&gt;</i> and <i>&lt;head&gt;</i> tags as well as any whitespace.</p><p>Once the cookies have been set, they can be accessed on the next page load with the $_COOKIE array. Cookie values may also exist in $_REQUEST.</p>
	 * @param string $name <p>The name of the cookie.</p>
	 * @param string $value <p>The value of the cookie. This value is stored on the clients computer; do not store sensitive information. Assuming the <code>name</code> is <i>'cookiename'</i>, this value is retrieved through $_COOKIE['cookiename']</p>
	 * @param int $expire <p>The time the cookie expires. This is a Unix timestamp so is in number of seconds since the epoch. In other words, you'll most likely set this with the <code>time()</code> function plus the number of seconds before you want it to expire. Or you might use <code>mktime()</code>. <i>time()+60&#42;60&#42;24&#42;30</i> will set the cookie to expire in 30 days. If set to 0, or omitted, the cookie will expire at the end of the session (when the browser closes).</p> <p></p><p><b>Note</b>:</p><p>You may notice the <code>expire</code> parameter takes on a Unix timestamp, as opposed to the date format <i>Wdy, DD-Mon-YYYY HH:MM:SS GMT</i>, this is because PHP does this conversion internally.</p>
	 * @param string $path <p>The path on the server in which the cookie will be available on. If set to <i>'/'</i>, the cookie will be available within the entire <code>domain</code>. If set to <i>'/foo/'</i>, the cookie will only be available within the <i>/foo/</i> directory and all sub-directories such as <i>/foo/bar/</i> of <code>domain</code>. The default value is the current directory that the cookie is being set in.</p>
	 * @param string $domain <p>The (sub)domain that the cookie is available to. Setting this to a subdomain (such as <i>'www.example.com'</i>) will make the cookie available to that subdomain and all other sub-domains of it (i.e. w2.www.example.com). To make the cookie available to the whole domain (including all subdomains of it), simply set the value to the domain name (<i>'example.com'</i>, in this case).</p> <p>Older browsers still implementing the deprecated RFC 2109 may require a leading <i>.</i> to match all subdomains.</p>
	 ...
	 * @param bool $httponly <p>When <b><code>TRUE</code></b> the cookie will be made accessible only through the HTTP protocol. This means that the cookie won't be accessible by scripting languages, such as JavaScript. It has been suggested that this setting can effectively help to reduce identity theft through XSS attacks (although it is not supported by all browsers), but that claim is often disputed. Added in PHP 5.2.0. <b><code>TRUE</code></b> or <b><code>FALSE</code></b></p>
	 ...
	 * @param string $message <p>The message to send, except that the two characters <i>%m</i> will be replaced by the error message string (strerror) corresponding to the present value of errno.</p>
	 * @return bool <p>Returns <b><code>TRUE</code></b> on success or <b><code>FALSE</code></b> on failure.</p>
	...
	/**
	 * Host Info Resource (See IANA's <i>Operating System Names</i> for the meaning of these values)
	 */
