...
namespace {
	 ...
	 * @see headers_sent(), setcookie(), http_response_code(), header_remove(), headers_list()
	 * @since PHP 4, PHP 5, PHP 7, PHP 8
	 */
	 ...
	 * @param int $expires_or_options <p>The time the cookie expires. This is a Unix timestamp so is in number of seconds since the epoch. One way to set this is by adding the number of seconds before the cookie should expire to the result of calling <code>time()</code>. For instance, <code>time()+60&#42;60&#42;24&#42;30</code> will set the cookie to expire in 30 days. Another option is to use the <code>mktime()</code> function. If set to <code>0</code>, or omitted, the cookie will expire at the end of the session (when the browser closes).</p> <p></p><p><b>Note</b>:</p><p>You may notice the <code>expires_or_options</code> parameter takes on a Unix timestamp, as opposed to the date format <code>Wdy, DD-Mon-YYYY HH:MM:SS GMT</code>, this is because PHP does this conversion internally.</p>
