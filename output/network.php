...
namespace {
	 ...
	 * @param float|null $timeout <p>The connection timeout, in seconds.</p> <p><b>Note</b>:</p><p>If you need to set a timeout for reading/writing data over the socket, use <code>stream_set_timeout()</code>, as the <code>timeout</code> parameter to <b>fsockopen()</b> only applies while connecting the socket.</p>
	 ...
	 * <p>Alternative signature available as of PHP 7.3.0:</p><p><b>setrawcookie()</b> is exactly the same as <code>setcookie()</code> except that the cookie value will not be automatically urlencoded when sent to the browser.</p>
	 * @param string $name
	 * @param string $value
	 * @param int $expires
	 * @param string $path
