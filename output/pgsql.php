...
namespace {
	 ...
	 * <p><b>pg_affected_rows()</b> returns the number of tuples (instances/records/rows) affected by <code>INSERT</code>, <code>UPDATE</code>, and <code>DELETE</code> queries.</p><p>Since PostgreSQL 9.0 and above, the server returns the number of SELECTed rows. Older PostgreSQL return 0 for SELECT.</p><p><b>Note</b>:</p><p>This function used to be called <b>pg_cmdtuples()</b>.</p>
	 ...
	 * @return int <p>The number of rows affected by the query. If no tuple is affected, it will return <code>0</code>.</p>
	 ...
	 * @link https://php.net/manual/en/function.pg-consume-input.php
	 * @since PHP 5 >= 5.6.0, PHP 7, PHP 8
	 */
	 ...
	 * <p><b>pg_convert()</b> checks and converts the values in <code>values</code> into suitable values for use in an SQL statement. Precondition for <b>pg_convert()</b> is the existence of a table <code>table_name</code> which has at least as many columns as <code>values</code> has elements. The fieldnames in <code>table_name</code> must match the indices in <code>values</code> and the corresponding datatypes must be compatible. Returns an array with the converted values on success, <b><code>false</code></b> otherwise.</p><p><b>Note</b>:</p><p>Boolean values are accepted and converted to PostgreSQL booleans. String representations of boolean values are also supported. <b><code>null</code></b> is converted to PostgreSQL NULL.</p>
	 * @param \PgSql\Connection $connection <p>An <code>PgSql\Connection</code> instance.</p>
	 * @param string $table_name <p>Name of the table against which to convert types.</p>
	 * @param array $values <p>Data to be converted.</p>
	 * @param int $flags <p>Any number of <b><code>PGSQL_CONV_IGNORE_DEFAULT</code></b>, <b><code>PGSQL_CONV_FORCE_NULL</code></b> or <b><code>PGSQL_CONV_IGNORE_NOT_NULL</code></b>, combined.</p>
	 * @return array|false <p>An <code>array</code> of converted values, or <b><code>false</code></b> on failure.</p>
	 ...
	 * <p><b>pg_copy_from()</b> inserts records into a table from <code>rows</code>. It issues a <code>COPY FROM</code> SQL command internally to insert records.</p>
	 * @param \PgSql\Connection $connection <p>An <code>PgSql\Connection</code> instance.</p>
	 ...
	 * @param string $separator <p>The token that separates values for each field in each element of <code>rows</code>. Default is <code>\t</code>.</p>
	 * @param string $null_as <p>How SQL <code>NULL</code> values are represented in the <code>rows</code>. Default is <code>\\N</code> (<code>"\\\\N"</code>).</p>
	 * @return bool <p>Returns <b><code>true</code></b> on success or <b><code>false</code></b> on failure.</p>
	 ...
	 * <p><b>pg_copy_to()</b> copies a table to an array. It issues <code>COPY TO</code> SQL command internally to retrieve records.</p>
	 * @param \PgSql\Connection $connection <p>An <code>PgSql\Connection</code> instance.</p>
	 * @param string $table_name <p>Name of the table from which to copy the data into <code>rows</code>.</p>
	 * @param string $separator <p>The token that separates values for each field in each element of <code>rows</code>. Default is <code>\t</code>.</p>
	 * @param string $null_as <p>How SQL <code>NULL</code> values are represented in the <code>rows</code>. Default is <code>\\N</code> (<code>"\\\\N"</code>).</p>
	 * @return array|false <p>An <code>array</code> with one element for each line of <code>COPY</code> data, or <b><code>false</code></b> on failure.</p>
	...
	/**
	 * Deletes records
	 * <p><b>pg_delete()</b> deletes records from a table specified by the keys and values in <code>conditions</code>.</p><p>If <code>flags</code> is specified, <code>pg_convert()</code> is applied to <code>conditions</code> with the specified flags.</p><p>By default <b>pg_delete()</b> passes raw values. Values must be escaped or the <b><code>PGSQL_DML_ESCAPE</code></b> flag must be specified in <code>flags</code>. <b><code>PGSQL_DML_ESCAPE</code></b> quotes and escapes parameters/identifiers. Therefore, table/column names become case sensitive.</p><p>Note that neither escape nor prepared query can protect LIKE query, JSON, Array, Regex, etc. These parameters should be handled according to their contexts. i.e. Escape/validate values.</p>
	 * @param \PgSql\Connection $connection <p>An <code>PgSql\Connection</code> instance.</p>
	 * @param string $table_name <p>Name of the table from which to delete rows.</p>
	 * @param array $conditions <p>An <code>array</code> whose keys are field names in the table <code>table_name</code>, and whose values are the values of those fields that are to be deleted.</p>
	 * @param int $flags <p>Any number of <b><code>PGSQL_CONV_FORCE_NULL</code></b>, <b><code>PGSQL_DML_NO_CONV</code></b>, <b><code>PGSQL_DML_ESCAPE</code></b>, <b><code>PGSQL_DML_EXEC</code></b>, <b><code>PGSQL_DML_ASYNC</code></b> or <b><code>PGSQL_DML_STRING</code></b> combined. If <b><code>PGSQL_DML_STRING</code></b> is part of the <code>flags</code> then query string is returned. When <b><code>PGSQL_DML_NO_CONV</code></b> or <b><code>PGSQL_DML_ESCAPE</code></b> is set, it does not call <code>pg_convert()</code> internally.</p>
	 * @return string|bool <p>Returns <b><code>true</code></b> on success or <b><code>false</code></b> on failure. Returns <code>string</code> if <b><code>PGSQL_DML_STRING</code></b> is passed via <code>flags</code>.</p>
	 ...
	 * <p><b>pg_fetch_all_columns()</b> returns an array that contains all rows (records) in a particular column of the <code>PgSql\Result</code> instance.</p><p><b>Note</b>: This function sets NULL fields to the PHP <b><code>null</code></b> value.</p>
	 * @param \PgSql\Result $result <p>An <code>PgSql\Result</code> instance, returned by <code>pg_query()</code>, <code>pg_query_params()</code> or <code>pg_execute()</code>(among others).</p>
	 * @param int $field <p>Column number. Defaults to the first column if not specified.</p>
	 * @return array <p>An <code>array</code> with all values in the result column.</p>
	 ...
	 * @param \PgSql\Result $result <p>An <code>PgSql\Result</code> instance, returned by <code>pg_query()</code>, <code>pg_query_params()</code> or <code>pg_execute()</code>(among others).</p>
	 * @param ?int $row <p>Row number in result to fetch. Rows are numbered from 0 upwards. If omitted or <b><code>null</code></b>, the next row is fetched.</p>
	 * @return array|false <p>An <code>array</code> indexed associatively (by field name). Each value in the <code>array</code> is represented as a <code>string</code>. Database <code>NULL</code> values are returned as <b><code>null</code></b>.</p><p><b><code>false</code></b> is returned if <code>row</code> exceeds the number of rows in the set, there are no more rows, or on any other error.</p>
	 ...
	 * <p><b>pg_fetch_result()</b> returns the value of a particular row and field (column) in an <code>PgSql\Result</code> instance.</p><p><b>Note</b>:</p><p>This function used to be called <b>pg_result()</b>.</p>
	 * @param \PgSql\Result $result <p>An <code>PgSql\Result</code> instance, returned by <code>pg_query()</code>, <code>pg_query_params()</code> or <code>pg_execute()</code>(among others).</p>
	 ...
	 * @return string|false|null <p>Boolean is returned as "t" or "f". All other types, including arrays are returned as strings formatted in the same default PostgreSQL manner that you would see in the <b>psql</b> program. Database <code>NULL</code> values are returned as <b><code>null</code></b>.</p><p><b><code>false</code></b> is returned if <code>row</code> exceeds the number of rows in the set, or on any other error.</p>
	 ...
	 * <p><b>pg_field_is_null()</b> tests if a field in an <code>PgSql\Result</code> instance is SQL <code>NULL</code> or not.</p><p><b>Note</b>:</p><p>This function used to be called <b>pg_fieldisnull()</b>.</p>
	 * @param \PgSql\Result $result <p>An <code>PgSql\Result</code> instance, returned by <code>pg_query()</code>, <code>pg_query_params()</code> or <code>pg_execute()</code>(among others).</p>
	 * @param int $row <p>Row number in result to fetch. Rows are numbered from 0 upwards. If omitted, current row is fetched.</p>
	 * @param mixed $field <p>Field number (starting from 0) as an <code>int</code> or the field name as a <code>string</code>.</p>
	 * @return int <p>Returns <code>1</code> if the field in the given row is SQL <code>NULL</code>, <code>0</code> if not. <b><code>false</code></b> is returned if the row is out of range, or upon any other error.</p>
	 ...
	 * <p><b>pg_field_name()</b> returns the name of the field occupying the given <code>field</code> in the given <code>result</code> instance. Field numbering starts from 0.</p><p><b>Note</b>:</p><p>This function used to be called <b>pg_fieldname()</b>.</p>
	 * @param \PgSql\Result $result <p>An <code>PgSql\Result</code> instance, returned by <code>pg_query()</code>, <code>pg_query_params()</code> or <code>pg_execute()</code>(among others).</p>
	 * @param int $field <p>Field number, starting from 0.</p>
	 * @return string <p>The field name.</p>
	 ...
	 * <p><b>pg_field_num()</b> will return the number of the field number that corresponds to the <code>field</code> in the given <code>result</code> instance.</p><p><b>Note</b>:</p><p>This function used to be called <b>pg_fieldnum()</b>.</p>
	 * @param \PgSql\Result $result <p>An <code>PgSql\Result</code> instance, returned by <code>pg_query()</code>, <code>pg_query_params()</code> or <code>pg_execute()</code>(among others).</p>
	 * @param string $field <p>The name of the field. The given name is treated like an identifier in an SQL command, that is, it is downcased unless double-quoted.</p>
	 ...
	 * <p><b>pg_field_prtlen()</b> returns the actual printed length (number of characters) of a specific value in a PostgreSQL <code>result</code>. Row numbering starts at 0. This function will return <b><code>false</code></b> on an error.</p><p><code>field_name_or_number</code> can be passed either as an <code>int</code> or as a <code>string</code>. If it is passed as an <code>int</code>, PHP recognises it as the field number, otherwise as field name.</p><p>See the example given at the <code>pg_field_name()</code> page.</p><p><b>Note</b>:</p><p>This function used to be called <b>pg_fieldprtlen()</b>.</p>
	 * @param \PgSql\Result $result <p>An <code>PgSql\Result</code> instance, returned by <code>pg_query()</code>, <code>pg_query_params()</code> or <code>pg_execute()</code>(among others).</p>
	 * @param int $row_number
	 * @param mixed $field_name_or_number
	 * @return int <p>The field printed length.</p>
	 ...
	 * @param \PgSql\Result $result <p>An <code>PgSql\Result</code> instance, returned by <code>pg_query()</code>, <code>pg_query_params()</code> or <code>pg_execute()</code>(among others).</p>
	 * @param int $field <p>Field number, starting from 0.</p>
	 * @return int <p>The internal field storage size (in bytes). -1 indicates a variable length field.</p>
	 ...
	 * @return string|int|false <p>On success either the fields table name or oid, or <b><code>false</code></b> on failure.</p>
	 ...
	 * <p><b>pg_field_type()</b> returns a string containing the base type name of the given <code>field</code> in the given <code>result</code> instance.</p><p><b>Note</b>:</p><p>If the field uses a PostgreSQL domain (rather than a basic type), it is the name of the domain's underlying type that is returned, rather than the name of the domain itself.</p><p><b>Note</b>:</p><p>This function used to be called <b>pg_fieldtype()</b>.</p>
	 * @param \PgSql\Result $result <p>An <code>PgSql\Result</code> instance, returned by <code>pg_query()</code>, <code>pg_query_params()</code> or <code>pg_execute()</code>(among others).</p>
	 * @param int $field <p>Field number, starting from 0.</p>
	 * @return string <p>A <code>string</code> containing the base name of the field's type.</p>
	 ...
	 * <p><b>pg_field_type_oid()</b> returns an integer containing the OID of the base type of the given <code>field</code> in the given <code>result</code> instance.</p><p>You can get more information about the field type by querying PostgreSQL's <code>pg_type</code> system table using the OID obtained with this function. The PostgreSQL <b>format_type()</b> function will convert a type OID into an SQL standard type name.</p><p><b>Note</b>:</p><p>If the field uses a PostgreSQL domain (rather than a basic type), it is the OID of the domain's underlying type that is returned, rather than the OID of the domain itself.</p>
	 * @param \PgSql\Result $result <p>An <code>PgSql\Result</code> instance, returned by <code>pg_query()</code>, <code>pg_query_params()</code> or <code>pg_execute()</code>(among others).</p>
	 * @param int $field <p>Field number, starting from 0.</p>
	 * @return string|int <p>The OID of the field's base type.</p>
	 ...
	 * @param \PgSql\Connection $connection <p>An <code>PgSql\Connection</code> instance.</p>
	 * @return int|bool <p>Returns <b><code>true</code></b> if the flush was successful or no data was waiting to be flushed, <code>0</code> if part of the pending data was flushed but more remains or <b><code>false</code></b> on failure.</p>
	...
	/**
	 * Free result memory
	 * <p><b>pg_free_result()</b> frees the memory and data associated with the specified <code>PgSql\Result</code> instance.</p><p>This function need only be called if memory consumption during script execution is a problem. Otherwise, all result memory will be automatically freed when the script ends.</p><p><b>Note</b>:</p><p>This function used to be called <b>pg_freeresult()</b>.</p>
	 * @param \PgSql\Result $result <p>An <code>PgSql\Result</code> instance, returned by <code>pg_query()</code>, <code>pg_query_params()</code> or <code>pg_execute()</code>(among others).</p>
	 * @return bool <p>Returns <b><code>true</code></b> on success or <b><code>false</code></b> on failure.</p>
	 ...
	 * <p><b>pg_get_pid()</b> gets the backend's (database server process) PID. The PID is useful to determine whether or not a <code>NOTIFY</code> message received via <code>pg_get_notify()</code> is sent from another process or not.</p>
	 * @param \PgSql\Connection $connection <p>An <code>PgSql\Connection</code> instance.</p>
	 ...
	 * <p><b>pg_insert()</b> inserts the values of <code>values</code> into the table specified by <code>table_name</code>.</p><p>If <code>flags</code> is specified, <code>pg_convert()</code> is applied to <code>values</code> with the specified flags.</p><p>By default <b>pg_insert()</b> passes raw values. Values must be escaped or the <b><code>PGSQL_DML_ESCAPE</code></b> flag must be specified in <code>flags</code>. <b><code>PGSQL_DML_ESCAPE</code></b> quotes and escapes parameters/identifiers. Therefore, table/column names become case sensitive.</p><p>Note that neither escape nor prepared query can protect LIKE query, JSON, Array, Regex, etc. These parameters should be handled according to their contexts. i.e. Escape/validate values.</p>
	 * @param \PgSql\Connection $connection <p>An <code>PgSql\Connection</code> instance.</p>
	 * @param string $table_name <p>Name of the table into which to insert rows. The table <code>table_name</code> must at least have as many columns as <code>values</code> has elements.</p>
	 * @param array $values <p>An <code>array</code> whose keys are field names in the table <code>table_name</code>, and whose values are the values of those fields that are to be inserted.</p>
	 * @param int $flags <p>Any number of <b><code>PGSQL_CONV_OPTS</code></b>, <b><code>PGSQL_DML_NO_CONV</code></b>, <b><code>PGSQL_DML_ESCAPE</code></b>, <b><code>PGSQL_DML_EXEC</code></b>, <b><code>PGSQL_DML_ASYNC</code></b> or <b><code>PGSQL_DML_STRING</code></b> combined. If <b><code>PGSQL_DML_STRING</code></b> is part of the <code>flags</code> then query string is returned. When <b><code>PGSQL_DML_NO_CONV</code></b> or <b><code>PGSQL_DML_ESCAPE</code></b> is set, it does not call <code>pg_convert()</code> internally.</p>
	 * @return PgSql\Result|string|bool <p>Returns <b><code>true</code></b> on success or <b><code>false</code></b> on failure.. Or returns a <code>string</code> on success if <b><code>PGSQL_DML_STRING</code></b> is passed via <code>flags</code>.</p>
	 ...
	 * <p><b>pg_last_notice()</b> returns the last notice message from the PostgreSQL server on the specified <code>connection</code>. The PostgreSQL server sends notice messages in several cases, for instance when creating a <code>SERIAL</code> column in a table.</p><p>With <b>pg_last_notice()</b>, you can avoid issuing useless queries by checking whether or not the notice is related to your transaction.</p><p>Notice message tracking can be set to optional by setting 1 for <code>pgsql.ignore_notice</code> in php.ini.</p><p>Notice message logging can be set to optional by setting 0 for <code>pgsql.log_notice</code> in php.ini. Unless <code>pgsql.ignore_notice</code> is set to 0, notice message cannot be logged.</p>
	 * @param \PgSql\Connection $connection <p>An <code>PgSql\Connection</code> instance.</p>
	 * @param int $mode <p>One of <b><code>PGSQL_NOTICE_LAST</code></b> (to return last notice), <b><code>PGSQL_NOTICE_ALL</code></b> (to return all notices), or <b><code>PGSQL_NOTICE_CLEAR</code></b> (to clear notices).</p>
	 * @return array|string|bool <p>A <code>string</code> containing the last notice on the given <code>connection</code> with <b><code>PGSQL_NOTICE_LAST</code></b>, an <code>array</code> with <b><code>PGSQL_NOTICE_ALL</code></b>, a <code>bool</code> with <b><code>PGSQL_NOTICE_CLEAR</code></b>.</p>
	 ...
	 * <p><b>pg_last_oid()</b> is used to retrieve the OID assigned to an inserted row.</p><p>OID field became an optional field from PostgreSQL 7.2 and will not be present by default in PostgreSQL 8.1. When the OID field is not present in a table, the programmer must use <code>pg_result_status()</code> to check for successful insertion.</p><p>To get the value of a <code>SERIAL</code> field in an inserted row, it is necessary to use the PostgreSQL <code>CURRVAL</code> function, naming the sequence whose last value is required. If the name of the sequence is unknown, the <code>pg_get_serial_sequence</code> PostgreSQL 8.0 function is necessary.</p><p>PostgreSQL 8.1 has a function <code>LASTVAL</code> that returns the value of the most recently used sequence in the session. This avoids the need for naming the sequence, table or column altogether.</p><p><b>Note</b>:</p><p>This function used to be called <b>pg_getlastoid()</b>.</p>
	 * @param \PgSql\Result $result <p>An <code>PgSql\Result</code> instance, returned by <code>pg_query()</code>, <code>pg_query_params()</code> or <code>pg_execute()</code>(among others).</p>
	 * @return string|int|false <p>An <code>int</code> or <code>string</code> containing the OID assigned to the most recently inserted row in the specified <code>connection</code>, or <b><code>false</code></b> on error or no available OID.</p>
	 ...
	 * <p><b>pg_lo_close()</b> closes a large object.</p><p>To use the large object interface, it is necessary to enclose it within a transaction block.</p><p><b>Note</b>:</p><p>This function used to be called <b>pg_loclose()</b>.</p>
	 * @param \PgSql\Lob $lob <p>An <code>PgSql\Lob</code> instance, returned by <code>pg_lo_open()</code>.</p>
	 * @return bool <p>Returns <b><code>true</code></b> on success or <b><code>false</code></b> on failure.</p>
	...
	/**
	 * Read a large object
	 * <p><b>pg_lo_read()</b> reads at most <code>length</code> bytes from a large object and returns it as a <code>string</code>.</p><p>To use the large object interface, it is necessary to enclose it within a transaction block.</p><p><b>Note</b>:</p><p>This function used to be called <b>pg_loread()</b>.</p>
	 * @param \PgSql\Lob $lob <p>An <code>PgSql\Lob</code> instance, returned by <code>pg_lo_open()</code>.</p>
	 * @param int $length <p>An optional maximum number of bytes to return.</p>
	 * @return string|false <p>A <code>string</code> containing <code>length</code> bytes from the large object, or <b><code>false</code></b> on error.</p>
	 ...
	 * @param \PgSql\Lob $lob <p>An <code>PgSql\Lob</code> instance, returned by <code>pg_lo_open()</code>.</p>
	 * @return int <p>Number of bytes read.</p>
	 ...
	 * <p><b>pg_lo_seek()</b> seeks a position within an <code>PgSql\Lob</code> instance.</p><p>To use the large object interface, it is necessary to enclose it within a transaction block.</p>
	 * @param \PgSql\Lob $lob <p>An <code>PgSql\Lob</code> instance, returned by <code>pg_lo_open()</code>.</p>
	 ...
	 * <p><b>pg_lo_truncate()</b> truncates an <code>PgSql\Lob</code> instance.</p><p>To use the large object interface, it is necessary to enclose it within a transaction block.</p>
	 * @param \PgSql\Lob $lob <p>An <code>PgSql\Lob</code> instance, returned by <code>pg_lo_open()</code>.</p>
	 * @param int $size <p>The number of bytes to truncate.</p>
	 * @return bool <p>Returns <b><code>true</code></b> on success or <b><code>false</code></b> on failure.</p>
	 * @link https://php.net/manual/en/function.pg-lo-truncate.php
	 * @see pg_lo_tell()
	 * @since PHP 5 >= 5.6.0, PHP 7, PHP 8
	 */
	 ...
	 * @param \PgSql\Lob $lob <p>An <code>PgSql\Lob</code> instance, returned by <code>pg_lo_open()</code>.</p>
	 * @param string $data <p>The data to be written to the large object. If <code>length</code> is an <code>int</code> and is less than the length of <code>data</code>, only <code>length</code> bytes will be written.</p>
	 * @param ?int $length <p>An optional maximum number of bytes to write. Must be greater than zero and no greater than the length of <code>data</code>. Defaults to the length of <code>data</code>.</p>
	 * @return int|false <p>The number of bytes written to the large object, or <b><code>false</code></b> on error.</p>
	 ...
	 * <p><b>pg_meta_data()</b> returns table definition for <code>table_name</code> as an array.</p>
	 * @param \PgSql\Connection $connection <p>An <code>PgSql\Connection</code> instance.</p>
	 ...
	 * @return array|false <p>An <code>array</code> of the table definition, or <b><code>false</code></b> on failure.</p>
	 ...
	 * <p><b>pg_num_fields()</b> returns the number of fields (columns) in the <code>PgSql\Result</code> instance.</p><p><b>Note</b>:</p><p>This function used to be called <b>pg_numfields()</b>.</p>
	 * @param \PgSql\Result $result <p>An <code>PgSql\Result</code> instance, returned by <code>pg_query()</code>, <code>pg_query_params()</code> or <code>pg_execute()</code>(among others).</p>
	 ...
	 * <p><b>pg_num_rows()</b> will return the number of rows in an <code>PgSql\Result</code> instance.</p><p><b>Note</b>:</p><p>This function used to be called <b>pg_numrows()</b>.</p>
	 * @param \PgSql\Result $result <p>An <code>PgSql\Result</code> instance, returned by <code>pg_query()</code>, <code>pg_query_params()</code> or <code>pg_execute()</code>(among others).</p>
	 * @return int <p>The number of rows in the result. On error, <code>-1</code> is returned.</p>
	 ...
	 * <p><b>pg_pconnect()</b> opens a connection to a PostgreSQL database. It returns an <code>PgSql\Connection</code> instance that is needed by other PostgreSQL functions.</p><p>If a second call is made to <b>pg_pconnect()</b> with the same <code>connection_string</code> as an existing connection, the existing connection will be returned unless you pass <b><code>PGSQL_CONNECT_FORCE_NEW</code></b> as <code>flags</code>.</p><p>To enable persistent connection, the pgsql.allow_persistent php.ini directive must be set to "On" (which is the default). The maximum number of persistent connection can be defined with the pgsql.max_persistent php.ini directive (defaults to -1 for no limit). The total number of connections can be set with the pgsql.max_links php.ini directive.</p><p><code>pg_close()</code> will not close persistent links generated by <b>pg_pconnect()</b>.</p>
	 * @param string $connection_string <p>The <code>connection_string</code> can be empty to use all default parameters, or it can contain one or more parameter settings separated by whitespace. Each parameter setting is in the form <code>keyword = value</code>. Spaces around the equal sign are optional. To write an empty value or a value containing spaces, surround it with single quotes, e.g., <code>keyword = 'a value'</code>. Single quotes and backslashes within the value must be escaped with a backslash, i.e., \' and \\.</p> <p>The currently recognized parameter keywords are: <code>host</code>, <code>hostaddr</code>, <code>port</code>, <code>dbname</code>, <code>user</code>, <code>password</code>, <code>connect_timeout</code>, <code>options</code>, <code>tty</code> (ignored), <code>sslmode</code>, <code>requiressl</code> (deprecated in favor of <code>sslmode</code>), and <code>service</code>. Which of these arguments exist depends on your PostgreSQL version.</p>
	 * @param int $flags <p>If <b><code>PGSQL_CONNECT_FORCE_NEW</code></b> is passed, then a new connection is created, even if the <code>connection_string</code> is identical to an existing connection.</p>
	 * @return PgSql\Connection|false <p>Returns an <code>PgSql\Connection</code> instance on success, or <b><code>false</code></b> on failure.</p>
	 ...
	 * <p><b>pg_result_error()</b> returns any error message associated with the <code>result</code> instance. Therefore, the user has a better chance of getting the correct error message than with <code>pg_last_error()</code>.</p><p>The function <code>pg_result_error_field()</code> can give much greater detail on result errors than <b>pg_result_error()</b>.</p><p>Because <code>pg_query()</code> returns <b><code>false</code></b> if the query fails, you must use <code>pg_send_query()</code> and <code>pg_get_result()</code> to get the result handle.</p>
	 * @param \PgSql\Result $result <p>An <code>PgSql\Result</code> instance, returned by <code>pg_query()</code>, <code>pg_query_params()</code> or <code>pg_execute()</code>(among others).</p>
	 * @return string|false <p>Returns a <code>string</code>. Returns empty string if there is no error. If there is an error associated with the <code>result</code> parameter, returns <b><code>false</code></b>.</p>
	 ...
	 * <p><b>pg_result_seek()</b> sets the internal row offset in the <code>result</code> instance.</p>
	 * @param \PgSql\Result $result <p>An <code>PgSql\Result</code> instance, returned by <code>pg_query()</code>, <code>pg_query_params()</code> or <code>pg_execute()</code>(among others).</p>
	 * @param int $row <p>Row to move the internal offset to in the <code>PgSql\Result</code> instance. Rows are numbered starting from zero.</p>
	 * @return bool <p>Returns <b><code>true</code></b> on success or <b><code>false</code></b> on failure.</p>
	 ...
	 * <p><b>pg_result_status()</b> returns the status of the <code>PgSql\Result</code> instance, or the PostgreSQL command completion tag associated with the result</p>
	 * @param \PgSql\Result $result <p>An <code>PgSql\Result</code> instance, returned by <code>pg_query()</code>, <code>pg_query_params()</code> or <code>pg_execute()</code>(among others).</p>
	 * @param int $mode <p>Either <b><code>PGSQL_STATUS_LONG</code></b> to return the numeric status of the <code>result</code>, or <b><code>PGSQL_STATUS_STRING</code></b> to return the command tag of the <code>result</code>. If not specified, <b><code>PGSQL_STATUS_LONG</code></b> is the default.</p>
	 * @return string|int <p>Possible return values are <b><code>PGSQL_EMPTY_QUERY</code></b>, <b><code>PGSQL_COMMAND_OK</code></b>, <b><code>PGSQL_TUPLES_OK</code></b>, <b><code>PGSQL_COPY_OUT</code></b>, <b><code>PGSQL_COPY_IN</code></b>, <b><code>PGSQL_BAD_RESPONSE</code></b>, <b><code>PGSQL_NONFATAL_ERROR</code></b> and <b><code>PGSQL_FATAL_ERROR</code></b> if <b><code>PGSQL_STATUS_LONG</code></b> is specified. Otherwise, a <code>string</code> containing the PostgreSQL command tag is returned.</p>
	...
	/**
	 * Select records
	 * <p><b>pg_select()</b> selects records specified by <code>conditions</code> which has <code>field=&gt;value</code>. For a successful query, it returns an array containing all records and fields that match the condition specified by <code>conditions</code>.</p><p>If <code>flags</code> is specified, <code>pg_convert()</code> is applied to <code>conditions</code> with the specified flags.</p><p>By default <b>pg_select()</b> passes raw values. Values must be escaped or PGSQL_DML_ESCAPE option must be specified. PGSQL_DML_ESCAPE quotes and escapes parameters/identifiers. Therefore, table/column names became case sensitive.</p><p>Note that neither escape nor prepared query can protect LIKE query, JSON, Array, Regex, etc. These parameters should be handled according to their contexts. i.e. Escape/validate values.</p>
	 * @param \PgSql\Connection $connection <p>An <code>PgSql\Connection</code> instance.</p>
	 * @param string $table_name <p>Name of the table from which to select rows.</p>
	 * @param array $conditions <p>An <code>array</code> whose keys are field names in the table <code>table_name</code>, and whose values are the conditions that a row must meet to be retrieved.</p>
	 * @param int $flags <p>Any number of <b><code>PGSQL_CONV_FORCE_NULL</code></b>, <b><code>PGSQL_DML_NO_CONV</code></b>, <b><code>PGSQL_DML_ESCAPE</code></b>, <b><code>PGSQL_DML_EXEC</code></b>, <b><code>PGSQL_DML_ASYNC</code></b> or <b><code>PGSQL_DML_STRING</code></b> combined. If <b><code>PGSQL_DML_STRING</code></b> is part of the <code>flags</code> then query string is returned. When <b><code>PGSQL_DML_NO_CONV</code></b> or <b><code>PGSQL_DML_ESCAPE</code></b> is set, it does not call <code>pg_convert()</code> internally.</p>
	 * @param int $mode
	 * @return array|string|false <p>Returns <code>string</code> if <b><code>PGSQL_DML_STRING</code></b> is passed via <code>flags</code>, otherwise it returns an <code>array</code> on success, or <b><code>false</code></b> on failure.</p>
	 ...
	 * @param \PgSql\Connection $connection <p>An <code>PgSql\Connection</code> instance.</p>
	 * @param string $statement_name <p>The name of the prepared statement to execute. If "" is specified, then the unnamed statement is executed. The name must have been previously prepared using <code>pg_prepare()</code>, <code>pg_send_prepare()</code> or a <code>PREPARE</code> SQL command.</p>
	 ...
	 * @return int|bool <p>Returns <b><code>true</code></b> on success, <b><code>false</code></b> or <code>0</code> on failure. Use <code>pg_get_result()</code> to determine the query result.</p>
	 ...
	 * @param \PgSql\Connection $connection <p>An <code>PgSql\Connection</code> instance.</p>
	 * @param string $statement_name <p>The name to give the prepared statement. Must be unique per-connection. If "" is specified, then an unnamed statement is created, overwriting any previously defined unnamed statement.</p>
	 * @param string $query <p>The parameterized SQL statement. Must contain only a single statement. (multiple statements separated by semi-colons are not allowed.) If any parameters are used, they are referred to as $1, $2, etc.</p>
	 * @return int|bool <p>Returns <b><code>true</code></b> on success, <b><code>false</code></b> or <code>0</code> on failure. Use <code>pg_get_result()</code> to determine the query result.</p>
	 ...
	 * @param \PgSql\Connection $connection <p>An <code>PgSql\Connection</code> instance.</p>
	 * @return resource|false <p>A socket resource on success or <b><code>false</code></b> on failure.</p>
	 ...
	 * <p>Returns the current in-transaction status of the server.</p><p><b>pg_transaction_status()</b> will give incorrect results when using a PostgreSQL 7.3 server that has the parameter <code>autocommit</code> set to off. The server-side autocommit feature has been deprecated and does not exist in later server versions.</p>
	 * @param \PgSql\Connection $connection <p>An <code>PgSql\Connection</code> instance.</p>
	 ...
	 * <p><b>pg_unescape_bytea()</b> unescapes PostgreSQL bytea data values. It returns the unescaped string, possibly containing binary data.</p><p><b>Note</b>:</p><p>When you <code>SELECT</code> a bytea type, PostgreSQL returns octal byte values prefixed with '\' (e.g. \032). Users are supposed to convert back to binary format manually.</p><p>This function requires PostgreSQL 7.2 or later. With PostgreSQL 7.2.0 and 7.2.1, bytea values must be cast when you enable multi-byte support. i.e. <code>INSERT INTO test_table (image) VALUES ('$image_escaped'::bytea);</code> PostgreSQL 7.2.2 or later does not need a cast. The exception is when the client and backend character encoding does not match, and there may be multi-byte stream error. User must then cast to bytea to avoid this error.</p>
	 * @param string $string <p>A <code>string</code> containing PostgreSQL bytea data to be converted into a PHP binary string.</p>
	...
	/**
	 * Update table
	 * <p><b>pg_update()</b> updates records that matches <code>conditions</code> with <code>values</code>.</p><p>If <code>flags</code> is specified, <code>pg_convert()</code> is applied to <code>values</code> with the specified flags.</p><p>By default <b>pg_update()</b> passes raw values. Values must be escaped or the <b><code>PGSQL_DML_ESCAPE</code></b> flag must be specified in <code>flags</code>. <b><code>PGSQL_DML_ESCAPE</code></b> quotes and escapes parameters/identifiers. Therefore, table/column names become case sensitive.</p><p>Note that neither escape nor prepared query can protect LIKE query, JSON, Array, Regex, etc. These parameters should be handled according to their contexts. i.e. Escape/validate values.</p>
	 * @param \PgSql\Connection $connection <p>An <code>PgSql\Connection</code> instance.</p>
	 * @param string $table_name <p>Name of the table into which to update rows.</p>
	 * @param array $values <p>An <code>array</code> whose keys are field names in the table <code>table_name</code>, and whose values are what matched rows are to be updated to.</p>
	 * @param array $conditions <p>An <code>array</code> whose keys are field names in the table <code>table_name</code>, and whose values are the conditions that a row must meet to be updated.</p>
	 * @param int $flags <p>Any number of <b><code>PGSQL_CONV_FORCE_NULL</code></b>, <b><code>PGSQL_DML_NO_CONV</code></b>, <b><code>PGSQL_DML_ESCAPE</code></b>, <b><code>PGSQL_DML_EXEC</code></b>, <b><code>PGSQL_DML_ASYNC</code></b> or <b><code>PGSQL_DML_STRING</code></b> combined. If <b><code>PGSQL_DML_STRING</code></b> is part of the <code>flags</code> then query string is returned. When <b><code>PGSQL_DML_NO_CONV</code></b> or <b><code>PGSQL_DML_ESCAPE</code></b> is set, it does not call <code>pg_convert()</code> internally.</p>
	 * @return string|bool <p>Returns <b><code>true</code></b> on success or <b><code>false</code></b> on failure. Returns <code>string</code> if <b><code>PGSQL_DML_STRING</code></b> is passed via <code>flags</code>.</p>
	...
	/**
	 * Passed to <code>pg_connect()</code> to create an asynchronous connection.
	 */
	...
	define('PGSQL_CONNECTION_SSL_STARTUP', null);

	define('PGSQL_CONNECTION_STARTED', 2);

	/**
	 * Passed to <code>pg_convert()</code>. Use SQL <code>NULL</code> in place of an empty <code>string</code>.
	 */
	...
	/**
	 * Passed to <code>pg_convert()</code>. Ignore conversion of <b><code>null</code></b> into SQL <code>NOT NULL</code> columns.
	 */
	...
	/**
	 * Passed to <code>pg_result_error_field()</code>. The severity; the field contents are <code>ERROR</code>, <code>FATAL</code>, or <code>PANIC</code> (in an error message), or <code>WARNING</code>, <code>NOTICE</code>, <code>DEBUG</code>, <code>INFO</code>, or <code>LOG</code> (in a notice message), or a localized translation of one of these. Always present.
	 */
	...
	/**
	 * Returned by <code>pg_result_status()</code>. Successful completion of a command returning data (such as a <code>SELECT</code> or <code>SHOW</code>).
	 */
...
namespace PgSql {
	...
	/**
	 * <p>A fully opaque class which replaces a <code>pgsql link</code> resource as of PHP 8.1.0.</p>
	 * @link https://php.net/manual/en/class.pgsql-connection.php
	 * @since PHP 8 >= 8.1.0
	 */
	...
	/**
	 * <p>A fully opaque class which replaces a <code>pgsql large object</code> resource as of PHP 8.1.0.</p>
	 * @link https://php.net/manual/en/class.pgsql-lob.php
	 * @since PHP 8 >= 8.1.0
	 */
	final class Lob {
	}
	...
	/**
	 * <p>A fully opaque class which replaces a <code>pgsql result</code> resource as of PHP 8.1.0.</p>
	 * @link https://php.net/manual/en/class.pgsql-result.php
	 * @since PHP 8 >= 8.1.0
	 */
	final class Result {
	}
