...
namespace {
	 ...
	 * @param string $constant_name <p>The name of the constant.</p> <p><b>Note</b>:</p><p>It is possible to <b>define()</b> constants with reserved or even invalid names, whose value can (only) be retrieved with <code>constant()</code>. However, doing so is not recommended.</p>
	 ...
	 * @param bool $case_insensitive <p>If set to <b><code>true</code></b>, the constant will be defined case-insensitive. The default behavior is case-sensitive; i.e. <code>CONSTANT</code> and <code>Constant</code> represent different values.</p> <p><b>Warning</b></p> <p>Defining case-insensitive constants is deprecated as of PHP 7.3.0. As of PHP 8.0.0, only <code>false</code> is an acceptable value, passing <code>true</code> will produce a warning.</p>  <p><b>Note</b>:</p><p>Case-insensitive constants are stored as lower-case.</p>
	 ...
	 * @param string $constant_name <p>The constant name.</p>
	 * @return bool <p>Returns <b><code>true</code></b> if the named constant given by <code>constant_name</code> has been defined, <b><code>false</code></b> otherwise.</p>
	 ...
	 * @return string|bool <p>If <code>return</code> is set to <b><code>true</code></b>, returns the highlighted code as a string instead of printing it out. Otherwise, it will return <b><code>true</code></b> on success, <b><code>false</code></b> on failure.</p>
	 ...
	 * @param string $string <p>The PHP code to be highlighted. This should include the opening tag.</p>
	 * @param bool $return <p>Set this parameter to <b><code>true</code></b> to make this function return the highlighted code.</p>
	 * @return string|bool <p>If <code>return</code> is set to <b><code>true</code></b>, returns the highlighted code as a string instead of printing it out. Otherwise, it will return <b><code>true</code></b> on success, <b><code>false</code></b> on failure.</p>
	 ...
	 * @param bool $as_number <p>Whether the high resolution time should be returned as <code>array</code> or number.</p>
	 * @return array|int|float|false <p>Returns an array of integers in the form [seconds, nanoseconds], if the parameter <code>as_number</code> is false. Otherwise the nanoseconds are returned as <code>int</code> (64bit platforms) or <code>float</code> (32bit platforms). Returns <b><code>false</code></b> on failure.</p>
	 ...
	 * <p>Sets whether a client disconnect should cause a script to be aborted.</p><p>When running PHP as a command line script, and the script's tty goes away without the script being terminated then the script will die the next time it tries to write anything, unless <code>enable</code> is set to <b><code>true</code></b></p>
	 * @param ?bool $enable <p>If set and not <b><code>null</code></b>, this function will set the ignore_user_abort ini setting to the given <code>enable</code>. Otherwise, this function will only return the previous setting without changing it.</p>
	 ...
	 * @return string <p>The stripped source code will be returned on success, or an empty string on failure.</p><p><b>Note</b>:</p><p>This function respects the value of the short_open_tag ini directive.</p>
	 ...
	 * @param int $codepage <p>A codepage identifier.</p>
	 * @return bool <p>Returns <b><code>true</code></b> on success or <b><code>false</code></b> on failure.</p>
	 ...
	 * @param ?callable $handler <p>A callback function to set or remove. If set, this function will be called whenever a <code>CTRL+C</code> or <code>CTRL+BREAK</code> event occurs. The function is supposed to have the following signature:</p> <b>handler</b>(<code>int</code> <code>$event</code>): void   <code>event</code>   The <code>CTRL</code> event which has been received; either <b><code>PHP_WINDOWS_EVENT_CTRL_C</code></b> or <b><code>PHP_WINDOWS_EVENT_CTRL_BREAK</code></b>.    Setting a <b><code>null</code></b> <code>handler</code> causes the process to ignore <code>CTRL+C</code> events, but not <code>CTRL+BREAK</code> events.
	 ...
	 * <p>Returns three samples representing the average system load (the number of processes in the system run queue) over the last 1, 5 and 15 minutes, respectively. Returns <b><code>false</code></b> on failure.</p>
	 * @return array|false <p>Returns an <code>array</code> with three samples (last 1, 5 and 15 minutes).</p>
	 ...
	 * @return array|bool <p>Returns <b><code>true</code></b> on success or <b><code>false</code></b> on failure.</p><p>If the delay was interrupted by a signal, an associative array will be returned with the components:</p><ul> <li>  <code>seconds</code> - number of seconds remaining in the delay  </li> <li>  <code>nanoseconds</code> - number of nanoseconds remaining in the delay  </li> </ul>
	 ...
	 * <p>Unpacks from a binary string into an array according to the given <code>format</code>.</p><p>The unpacked data is stored in an associative array. To accomplish this you have to name the different format codes and separate them by a slash /. If a repeater argument is present, then each of the array keys will have a sequence number behind the given name.</p><p>Changes were made to bring this function into line with Perl:</p>
	 * @param string $format <p>See <code>pack()</code> for an explanation of the format codes.</p>
	 * @param string $string <p>The packed data.</p>
...
namespace HRTime {
	...
	class StopWatch extends \HRTime\PerformanceCounter {
		 ...
		 * @return bool <p>Returns <code>bool</code> indicating whether the measurement is running.</p>
