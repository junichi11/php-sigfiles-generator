...
namespace {
	 ...
	 * @param callback $callback <p>A callback which will be used whenever a query is being sent to MySQL. The callback will receive a single <code>boolean</code> parameter telling if a query was sent via Memcache.</p>
	...
	/**
	 * Default regular expression (PCRE style) used for matching <i>SELECT</i> statements that will be mapped into a MySQL Memcache Plugin access point, if possible.   It is also possible to use <code>mysqlnd_memcache_set()</code>, but the default approach is using this regular expression for pattern matching.
	 */
