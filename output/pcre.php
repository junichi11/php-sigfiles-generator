...
namespace {
	 ...
	 * @param int $limit
	 * @param int $count
	 * @return mixed <p>Returns an <code>array</code> if the <code>subject</code> parameter is an <code>array</code>, or a <code>string</code> otherwise.</p><p>If no matches are found or an error occurred, an empty <code>array</code> is returned when <code>subject</code> is an <code>array</code> or <b><code>NULL</code></b> otherwise.</p>
	 ...
	 * <p>Returns the array consisting of the elements of the <code>input</code> array that match the given <code>pattern</code>.</p>
	 ...
	 * @return array <p>Returns an array indexed using the keys from the <code>input</code> array.</p>
	 ...
	 * @return int <p>Returns the number of full pattern matches (which might be zero), or <b><code>FALSE</code></b> if an error occurred.</p>
	 ...
	 * <p><b>preg_quote()</b> takes <code>str</code> and puts a backslash in front of every character that is part of the regular expression syntax. This is useful if you have a run-time string that you need to match in some text and the string may contain special regex characters.</p><p>The special regular expression characters are: <i>. \ + &#42; &#63; [ ^ ] $ ( ) { } = ! &lt; &gt; | : -</i></p><p>Note that <i>/</i> is not a special regular expression character.</p><p><b>Note</b>:</p><p>Note that <b>preg_quote()</b> is not meant to be applied to the $replacement string(s) of <code>preg_replace()</code> etc.</p>
	 * @param string $str <p>The input string.</p>
	 * @param string $delimiter <p>If the optional <code>delimiter</code> is specified, it will also be escaped. This is useful for escaping the delimiter that is required by the PCRE functions. The <i>/</i> is the most commonly used delimiter.</p>
	 ...
	 * @param mixed $pattern <p>The pattern to search for. It can be either a string or an array with strings.</p> <p>Several PCRE modifiers are also available.</p>
	 * @param mixed $replacement <p>The string or an array with strings to replace. If this parameter is a string and the <code>pattern</code> parameter is an array, all patterns will be replaced by that string. If both <code>pattern</code> and <code>replacement</code> parameters are arrays, each <code>pattern</code> will be replaced by the <code>replacement</code> counterpart. If there are fewer elements in the <code>replacement</code> array than in the <code>pattern</code> array, any extra <code>pattern</code>s will be replaced by an empty string.</p> <p><code>replacement</code> may contain references of the form <i>\\n</i> or <i>$n</i>, with the latter form being the preferred one. Every such reference will be replaced by the text captured by the n'th parenthesized pattern. n can be from 0 to 99, and <i>\\0</i> or <i>$0</i> refers to the text matched by the whole pattern. Opening parentheses are counted from left to right (starting from 1) to obtain the number of the capturing subpattern. To use backslash in replacement, it must be doubled (<i>"\\\\"</i> PHP string).</p> <p>When working with a replacement pattern where a backreference is immediately followed by another number (i.e.: placing a literal number immediately after a matched pattern), you cannot use the familiar <i>\\1</i> notation for your backreference. <i>\\11</i>, for example, would confuse <b>preg_replace()</b> since it does not know whether you want the <i>\\1</i> backreference followed by a literal <i>1</i>, or the <i>\\11</i> backreference followed by nothing. In this case the solution is to use <i>${1}1</i>. This creates an isolated <i>$1</i> backreference, leaving the <i>1</i> as a literal.</p> <p>When using the deprecated <i>e</i> modifier, this function escapes some characters (namely <i>'</i>, <i>"</i>, <i>\</i> and NULL) in the strings that replace the backreferences. This is done to ensure that no syntax errors arise from backreference usage with either single or double quotes (e.g. <i>'strlen(\'$1\')+strlen("$2")'</i>). Make sure you are aware of PHP's string syntax to know exactly how the interpreted string will look.</p>
	 * @param mixed $subject <p>The string or an array with strings to search and replace.</p> <p>If <code>subject</code> is an array, then the search and replace is performed on every entry of <code>subject</code>, and the return value is an array as well.</p>
	 * @param int $limit <p>The maximum possible replacements for each pattern in each <code>subject</code> string. Defaults to <i>-1</i> (no limit).</p>
	 * @param int $count <p>If specified, this variable will be filled with the number of replacements done.</p>
	 * @return mixed <p><b>preg_replace()</b> returns an array if the <code>subject</code> parameter is an array, or a string otherwise.</p><p>If matches are found, the new <code>subject</code> will be returned, otherwise <code>subject</code> will be returned unchanged or <b><code>NULL</code></b> if an error occurred.</p>
	 ...
	 * @param mixed $pattern <p>The pattern to search for. It can be either a string or an array with strings.</p>
	 * @param callable $callback <p>A callback that will be called and passed an array of matched elements in the <code>subject</code> string. The callback should return the replacement string. This is the callback signature:</p> <p></p> string handler ( array <code>$matches</code> ) <p>You'll often need the <code>callback</code> function for a <b>preg_replace_callback()</b> in just one place. In this case you can use an anonymous function to declare the callback within the call to <b>preg_replace_callback()</b>. By doing it this way you have all information for the call in one place and do not clutter the function namespace with a callback function's name not used anywhere else.</p> <p></p> <p><b>Example #1 <b>preg_replace_callback()</b> and anonymous function</b></p>  <code> &lt;&#63;php<br>/&#42;&nbsp;a&nbsp;unix-style&nbsp;command&nbsp;line&nbsp;filter&nbsp;to&nbsp;convert&nbsp;uppercase<br>&nbsp;&#42;&nbsp;letters&nbsp;at&nbsp;the&nbsp;beginning&nbsp;of&nbsp;paragraphs&nbsp;to&nbsp;lowercase&nbsp;&#42;/<br>$fp&nbsp;=&nbsp;fopen("php://stdin",&nbsp;"r")&nbsp;or&nbsp;die("can't&nbsp;read&nbsp;stdin");<br>while&nbsp;(!feof($fp))&nbsp;{<br>&nbsp;&nbsp;&nbsp;&nbsp;$line&nbsp;=&nbsp;fgets($fp);<br>&nbsp;&nbsp;&nbsp;&nbsp;$line&nbsp;=&nbsp;preg_replace_callback(<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'|&lt;p&gt;\s&#42;\w|',<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;function&nbsp;($matches)&nbsp;{<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;return&nbsp;strtolower($matches[0]);<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;},<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;$line<br>&nbsp;&nbsp;&nbsp;&nbsp;);<br>&nbsp;&nbsp;&nbsp;&nbsp;echo&nbsp;$line;<br>}<br>fclose($fp);<br>&#63;&gt;  </code>
	 * @param mixed $subject <p>The string or an array with strings to search and replace.</p>
	 * @param int $limit <p>The maximum possible replacements for each pattern in each <code>subject</code> string. Defaults to <i>-1</i> (no limit).</p>
	 * @param int $count <p>If specified, this variable will be filled with the number of replacements done.</p>
	 * @return mixed <p><b>preg_replace_callback()</b> returns an array if the <code>subject</code> parameter is an array, or a string otherwise. On errors the return value is <b><code>NULL</code></b></p><p>If matches are found, the new subject will be returned, otherwise <code>subject</code> will be returned unchanged.</p>
	 ...
	 * @param array $patterns_and_callbacks <p>An associative array mapping patterns (keys) to callbacks (values).</p>
	 * @param mixed $subject <p>The string or an array with strings to search and replace.</p>
	 * @param int $limit <p>The maximum possible replacements for each pattern in each <code>subject</code> string. Defaults to <i>-1</i> (no limit).</p>
	 * @param int $count <p>If specified, this variable will be filled with the number of replacements done.</p>
	 * @return mixed <p><b>preg_replace_callback_array()</b> returns an array if the <code>subject</code> parameter is an array, or a string otherwise. On errors the return value is <b><code>NULL</code></b></p><p>If matches are found, the new subject will be returned, otherwise <code>subject</code> will be returned unchanged.</p>
	 ...
	 * @param string $pattern <p>The pattern to search for, as a string.</p>
	 * @param string $subject <p>The input string.</p>
	 * @param int $limit <p>If specified, then only substrings up to <code>limit</code> are returned with the rest of the string being placed in the last substring. A <code>limit</code> of -1 or 0 means "no limit" and, as is standard across PHP, you can use <b><code>NULL</code></b> to skip to the <code>flags</code> parameter.</p>
	 * @param int $flags <p><code>flags</code> can be any combination of the following flags (combined with the <i>|</i> bitwise operator):</p>  <b><code>PREG_SPLIT_NO_EMPTY</code></b>   If this flag is set, only non-empty pieces will be returned by <b>preg_split()</b>.    <b><code>PREG_SPLIT_DELIM_CAPTURE</code></b>   If this flag is set, parenthesized expression in the delimiter pattern will be captured and returned as well.    <b><code>PREG_SPLIT_OFFSET_CAPTURE</code></b>  <p>If this flag is set, for every occurring match the appendant string offset will also be returned. Note that this changes the return value in an array where every element is an array consisting of the matched string at offset <i>0</i> and its string offset into <code>subject</code> at offset <i>1</i>.</p>
	 * @return array <p>Returns an array containing substrings of <code>subject</code> split along boundaries matched by <code>pattern</code>, or <b><code>FALSE</code></b> on failure.</p>
