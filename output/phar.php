...
namespace {
	...
	class Phar extends \RecursiveDirectoryIterator implements \Countable, \ArrayAccess {
		 ...
		 * <p><b>Note</b>:</p><p>This method requires the php.ini setting <code>phar.readonly</code> to be set to <code>0</code> in order to work for Phar objects. Otherwise, a PharException will be thrown.</p><p>Populate a phar archive from directory contents. The optional second parameter is a regular expression (pcre) that is used to exclude files. Any filename that matches the regular expression will be included, all others will be excluded. For more fine-grained control, use <code>Phar::buildFromIterator()</code>.</p>
		 ...
		 * <p><b>Note</b>:</p><p>This method requires the php.ini setting <code>phar.readonly</code> to be set to <code>0</code> in order to work for Phar objects. Otherwise, a PharException will be thrown.</p><p>Populate a phar archive from an iterator. Two styles of iterators are supported, iterators that map the filename within the phar to the name of a file on disk, and iterators like DirectoryIterator that return SplFileInfo objects. For iterators that return SplFileInfo objects, the second parameter is required.</p>
		 ...
		 * <p><b>Note</b>:</p><p>This method requires the php.ini setting <code>phar.readonly</code> to be set to <code>0</code> in order to work for Phar objects. Otherwise, a PharException will be thrown.</p><p>This method is used to convert a phar archive to another file format. For instance, it can be used to create a tar-based executable phar archive from a zip-based executable phar archive, or from an executable phar archive in the phar file format. In addition, it can be used to apply whole-archive compression to a tar or phar-based archive.</p><p>If no changes are specified, this method throws a BadMethodCallException.</p><p>If successful, the method creates a new archive on disk and returns a Phar object. The old archive is not removed from disk, and should be done manually after the process has finished.</p>
		 ...
		 * <p><b>Note</b>:</p><p>This method requires the php.ini setting <code>phar.readonly</code> to be set to <code>0</code> in order to work for Phar objects. Otherwise, a PharException will be thrown.</p><p>Copy a file internal to the phar archive to another new file within the phar. This is an object-oriented alternative to using <code>copy()</code> with the phar stream wrapper.</p>
		 ...
		 * @since PHP 5 >= 5.3.0, PHP 7, PECL phar >= 1.0.0
		 */
		public function offsetGet(string $offset): \PharFileInfo {}

		/**
		 * Set the contents of an internal file to those of an external file
		 * <p><b>Note</b>:</p><p>This method requires the php.ini setting <code>phar.readonly</code> to be set to <code>0</code> in order to work for Phar objects. Otherwise, a PharException will be thrown.</p><p>This is an implementation of the <code>ArrayAccess</code> interface allowing direct manipulation of the contents of a Phar archive using array access brackets. offsetSet is used for modifying an existing file, or adding a new file to a Phar archive.</p>
		 ...
		 * <p><b>Note</b>:</p><p>This method requires the php.ini setting <code>phar.readonly</code> to be set to <code>0</code> in order to work for Phar objects. Otherwise, a PharException will be thrown.</p><p>This method is a convenience method that combines the functionality of <code>Phar::createDefaultStub()</code> and <code>Phar::setStub()</code>.</p>
		 ...
		 * <p><b>Note</b>:</p><p>This method requires the php.ini setting <code>phar.readonly</code> to be set to <code>0</code> in order to work for Phar objects. Otherwise, a PharException will be thrown.</p><p>This method is used to add a PHP bootstrap loader stub to a new Phar archive, or to replace the loader stub in an existing Phar archive.</p><p>The loader stub for a Phar archive is used whenever an archive is included directly as in this example:</p><p>The loader is not accessed when including a file through the <code>phar</code> stream wrapper like so:</p>
	...
	class PharData extends \RecursiveDirectoryIterator implements \Countable, \ArrayAccess {
		 ...
		 * @param string $base_dir <p>The full or relative path to the directory that contains all files to add to the archive.</p>
		 * @param string $regex <p>An optional pcre regular expression that is used to filter the list of files. Only file paths matching the regular expression will be included in the archive.</p>
		 * @return array <p><code>Phar::buildFromDirectory()</code> returns an associative array mapping internal path of file to the full path of the file on the filesystem.</p>
		 ...
		 * <p><b>Note</b>:</p><p>This method requires the php.ini setting <code>phar.readonly</code> to be set to <code>0</code> in order to work for Phar objects. Otherwise, a PharException will be thrown.</p><p>This method is used to convert a non-executable tar or zip archive to an executable phar archive. Any of the three executable file formats (phar, tar or zip) can be used, and whole-archive compression can also be performed.</p><p>If no changes are specified, this method throws a BadMethodCallException.</p><p>If successful, the method creates a new archive on disk and returns a Phar object. The old archive is not removed from disk, and should be done manually after the process has finished.</p>
		 * @param int $format <p>This should be one of <code>Phar::PHAR</code>, <code>Phar::TAR</code>, or <code>Phar::ZIP</code>. If set to <b><code>null</code></b>, the existing file format will be preserved.</p>
		 * @param int $compression <p>This should be one of <code>Phar::NONE</code> for no whole-archive compression, <code>Phar::GZ</code> for zlib-based compression, and <code>Phar::BZ2</code> for bzip-based compression.</p>
		 * @param string $extension <p>This parameter is used to override the default file extension for a converted archive. Note that all zip- and tar-based phar archives must contain <code>.phar</code> in their file extension in order to be processed as a phar archive.</p> <p>If converting to a phar-based archive, the default extensions are <code>.phar</code>, <code>.phar.gz</code>, or <code>.phar.bz2</code> depending on the specified compression. For tar-based phar archives, the default extensions are <code>.phar.tar</code>, <code>.phar.tar.gz</code>, and <code>.phar.tar.bz2</code>. For zip-based phar archives, the default extension is <code>.phar.zip</code>.</p>
		 * @return Phar <p>The method returns a Phar object on success and throws an exception on failure.</p>
		...
		/**
		 * Sets phar archive meta-data
		 * <p><b>Note</b>:</p><p>This method requires the php.ini setting <code>phar.readonly</code> to be set to <code>0</code> in order to work for Phar objects. Otherwise, a PharException will be thrown.</p><p><code>Phar::setMetadata()</code> should be used to store customized data that describes something about the phar archive as a complete entity. <code>PharFileInfo::setMetadata()</code> should be used for file-specific meta-data. Meta-data can slow down the performance of loading a phar archive if the data is large.</p><p>Some possible uses for meta-data include specifying which file within the archive should be used to bootstrap the archive, or the location of a file manifest like PEAR's package.xml file. However, any useful data that describes the phar archive may be stored.</p>
		 * @param mixed $metadata <p>Any PHP variable containing information to store that describes the phar archive</p>
		 * @return void <p>No value is returned.</p>
