...
namespace {
	 ...
	 * <p>Returns a string with backslashes added before characters that need to be escaped. These characters are:</p><p>A use case of <b>addslashes()</b> is escaping the aforementioned characters in a string that is to be evaluated by PHP:</p><p>Prior to PHP 5.4.0, the PHP directive magic_quotes_gpc was <i>on</i> by default and it essentially ran <b>addslashes()</b> on all GET, POST and COOKIE data. <b>addslashes()</b> must not be used on strings that have already been escaped with magic_quotes_gpc, as the strings will be double escaped. <code>get_magic_quotes_gpc()</code> can be used to check if magic_quotes_gpc is <i>on</i>.</p><p>The <b>addslashes()</b> is sometimes incorrectly used to try to prevent SQL Injection. Instead, database-specific escaping functions and/or prepared statements should be used.</p>
	 * @param string $str <p>The string to be escaped.</p>
	 * @return string <p>Returns the escaped string.</p>
	 ...
	 * @param string $character_mask <p>You can also specify the characters you want to strip, by means of the <code>character_mask</code> parameter. Simply list all characters that you want to be stripped. With <i>..</i> you can specify a range of characters.</p>
	 * @return string
	 ...
	 * <p><b>convert_uudecode()</b> decodes a uuencoded string.</p><p><b>Note</b>:  <b>convert_uudecode()</b> neither accepts the <i>begin</i> nor the <i>end</i> line, which are part of uuencoded <i>files</i>. </p>
	 * @param string $data <p>The uuencoded data.</p>
	 * @return string <p>Returns the decoded data as a string or <b><code>FALSE</code></b> on failure.</p>
	...
	/**
	 * Uuencode a string
	 * <p><b>convert_uuencode()</b> encodes a string using the uuencode algorithm.</p><p>Uuencode translates all strings (including binary data) into printable characters, making them safe for network transmissions. Uuencoded data is about 35% larger than the original.</p><p><b>Note</b>:  <b>convert_uuencode()</b> neither produces the <i>begin</i> nor the <i>end</i> line, which are part of uuencoded <i>files</i>. </p>
	 * @param string $data <p>The data to be encoded.</p>
	 * @return string <p>Returns the uuencoded data or <b><code>FALSE</code></b> on failure.</p>
	 ...
	 * @param string $format <p>See <code>sprintf()</code> for a description of <code>format</code>.</p>
	 * @param mixed $args
	 * @param mixed $_
	 ...
	 * @return string <p>Returns the binary representation of the given data or <b><code>FALSE</code></b> on failure.</p>
	 ...
	 * @param string $string <p>The input string.</p>
	 * @param int $flags <p>A bitmask of one or more of the following flags, which specify how to handle quotes and which document type to use. The default is <i>ENT_COMPAT | ENT_HTML401</i>.</p> <b>Available <code>flags</code> constants</b>   Constant Name Description     <b><code>ENT_COMPAT</code></b> Will convert double-quotes and leave single-quotes alone.   <b><code>ENT_QUOTES</code></b> Will convert both double and single quotes.   <b><code>ENT_NOQUOTES</code></b> Will leave both double and single quotes unconverted.   <b><code>ENT_HTML401</code></b>  Handle code as HTML 4.01.    <b><code>ENT_XML1</code></b>  Handle code as XML 1.    <b><code>ENT_XHTML</code></b>  Handle code as XHTML.    <b><code>ENT_HTML5</code></b>  Handle code as HTML 5.
	 * @param string $encoding <p>An optional argument defining the encoding used when converting characters.</p> <p>If omitted, the default value of the <code>encoding</code> varies depending on the PHP version in use. In PHP 5.6 and later, the default_charset configuration option is used as the default value. PHP 5.4 and 5.5 will use <i>UTF-8</i> as the default. Earlier versions of PHP use <i>ISO-8859-1</i>.</p> <p>Although this argument is technically optional, you are highly encouraged to specify the correct value for your code if you are using PHP 5.5 or earlier, or if your default_charset configuration option may be set incorrectly for the given input.</p> <p>The following character sets are supported:</p> <b>Supported charsets</b>   Charset Aliases Description     ISO-8859-1 ISO8859-1  Western European, Latin-1.    ISO-8859-5 ISO8859-5  Little used cyrillic charset (Latin/Cyrillic).    ISO-8859-15 ISO8859-15  Western European, Latin-9. Adds the Euro sign, French and Finnish letters missing in Latin-1 (ISO-8859-1).    UTF-8 &nbsp;  ASCII compatible multi-byte 8-bit Unicode.    cp866 ibm866, 866  DOS-specific Cyrillic charset.    cp1251 Windows-1251, win-1251, 1251  Windows-specific Cyrillic charset.    cp1252 Windows-1252, 1252  Windows specific charset for Western European.    KOI8-R koi8-ru, koi8r  Russian.    BIG5 950  Traditional Chinese, mainly used in Taiwan.    GB2312 936  Simplified Chinese, national standard character set.    BIG5-HKSCS &nbsp;  Big5 with Hong Kong extensions, Traditional Chinese.    Shift_JIS SJIS, SJIS-win, cp932, 932  Japanese    EUC-JP EUCJP, eucJP-win  Japanese    MacRoman &nbsp;  Charset that was used by Mac OS.    <i>''</i> &nbsp;  An empty string activates detection from script encoding (Zend multibyte), default_charset and current locale (see <code>nl_langinfo()</code> and <code>setlocale()</code>), in this order. Not recommended.     <p><b>Note</b>:  Any other character sets are not recognized. The default encoding will be used instead and a warning will be emitted. </p>
	 ...
	 * @param string $string <p>The input string.</p>
	 * @param int $flags <p>A bitmask of one or more of the following flags, which specify how to handle quotes, invalid code unit sequences and the used document type. The default is <i>ENT_COMPAT | ENT_HTML401</i>.</p> <b>Available <code>flags</code> constants</b>   Constant Name Description     <b><code>ENT_COMPAT</code></b> Will convert double-quotes and leave single-quotes alone.   <b><code>ENT_QUOTES</code></b> Will convert both double and single quotes.   <b><code>ENT_NOQUOTES</code></b> Will leave both double and single quotes unconverted.   <b><code>ENT_IGNORE</code></b>  Silently discard invalid code unit sequences instead of returning an empty string. Using this flag is discouraged as it may have security implications.    <b><code>ENT_SUBSTITUTE</code></b>  Replace invalid code unit sequences with a Unicode Replacement Character U+FFFD (UTF-8) or &amp;#FFFD; (otherwise) instead of returning an empty string.    <b><code>ENT_DISALLOWED</code></b>  Replace invalid code points for the given document type with a Unicode Replacement Character U+FFFD (UTF-8) or &amp;#FFFD; (otherwise) instead of leaving them as is. This may be useful, for instance, to ensure the well-formedness of XML documents with embedded external content.    <b><code>ENT_HTML401</code></b>  Handle code as HTML 4.01.    <b><code>ENT_XML1</code></b>  Handle code as XML 1.    <b><code>ENT_XHTML</code></b>  Handle code as XHTML.    <b><code>ENT_HTML5</code></b>  Handle code as HTML 5.
	 * @param string $encoding <p>An optional argument defining the encoding used when converting characters.</p> <p>If omitted, the default value of the <code>encoding</code> varies depending on the PHP version in use. In PHP 5.6 and later, the default_charset configuration option is used as the default value. PHP 5.4 and 5.5 will use <i>UTF-8</i> as the default. Earlier versions of PHP use <i>ISO-8859-1</i>.</p> <p>Although this argument is technically optional, you are highly encouraged to specify the correct value for your code if you are using PHP 5.5 or earlier, or if your default_charset configuration option may be set incorrectly for the given input.</p> <p>The following character sets are supported:</p> <b>Supported charsets</b>   Charset Aliases Description     ISO-8859-1 ISO8859-1  Western European, Latin-1.    ISO-8859-5 ISO8859-5  Little used cyrillic charset (Latin/Cyrillic).    ISO-8859-15 ISO8859-15  Western European, Latin-9. Adds the Euro sign, French and Finnish letters missing in Latin-1 (ISO-8859-1).    UTF-8 &nbsp;  ASCII compatible multi-byte 8-bit Unicode.    cp866 ibm866, 866  DOS-specific Cyrillic charset.    cp1251 Windows-1251, win-1251, 1251  Windows-specific Cyrillic charset.    cp1252 Windows-1252, 1252  Windows specific charset for Western European.    KOI8-R koi8-ru, koi8r  Russian.    BIG5 950  Traditional Chinese, mainly used in Taiwan.    GB2312 936  Simplified Chinese, national standard character set.    BIG5-HKSCS &nbsp;  Big5 with Hong Kong extensions, Traditional Chinese.    Shift_JIS SJIS, SJIS-win, cp932, 932  Japanese    EUC-JP EUCJP, eucJP-win  Japanese    MacRoman &nbsp;  Charset that was used by Mac OS.    <i>''</i> &nbsp;  An empty string activates detection from script encoding (Zend multibyte), default_charset and current locale (see <code>nl_langinfo()</code> and <code>setlocale()</code>), in this order. Not recommended.     <p><b>Note</b>:  Any other character sets are not recognized. The default encoding will be used instead and a warning will be emitted. </p>
	 ...
	 * @param string $encoding <p>An optional argument defining the encoding used when converting characters.</p> <p>If omitted, the default value of the <code>encoding</code> varies depending on the PHP version in use. In PHP 5.6 and later, the default_charset configuration option is used as the default value. PHP 5.4 and 5.5 will use <i>UTF-8</i> as the default. Earlier versions of PHP use <i>ISO-8859-1</i>.</p> <p>Although this argument is technically optional, you are highly encouraged to specify the correct value for your code if you are using PHP 5.5 or earlier, or if your default_charset configuration option may be set incorrectly for the given input.</p> <p>For the purposes of this function, the encodings <i>ISO-8859-1</i>, <i>ISO-8859-15</i>, <i>UTF-8</i>, <i>cp866</i>, <i>cp1251</i>, <i>cp1252</i>, and <i>KOI8-R</i> are effectively equivalent, provided the <code>string</code> itself is valid for the encoding, as the characters affected by <b>htmlspecialchars()</b> occupy the same positions in all of these encodings.</p> <p>The following character sets are supported:</p> <b>Supported charsets</b>   Charset Aliases Description     ISO-8859-1 ISO8859-1  Western European, Latin-1.    ISO-8859-5 ISO8859-5  Little used cyrillic charset (Latin/Cyrillic).    ISO-8859-15 ISO8859-15  Western European, Latin-9. Adds the Euro sign, French and Finnish letters missing in Latin-1 (ISO-8859-1).    UTF-8 &nbsp;  ASCII compatible multi-byte 8-bit Unicode.    cp866 ibm866, 866  DOS-specific Cyrillic charset.    cp1251 Windows-1251, win-1251, 1251  Windows-specific Cyrillic charset.    cp1252 Windows-1252, 1252  Windows specific charset for Western European.    KOI8-R koi8-ru, koi8r  Russian.    BIG5 950  Traditional Chinese, mainly used in Taiwan.    GB2312 936  Simplified Chinese, national standard character set.    BIG5-HKSCS &nbsp;  Big5 with Hong Kong extensions, Traditional Chinese.    Shift_JIS SJIS, SJIS-win, cp932, 932  Japanese    EUC-JP EUCJP, eucJP-win  Japanese    MacRoman &nbsp;  Charset that was used by Mac OS.    <i>''</i> &nbsp;  An empty string activates detection from script encoding (Zend multibyte), default_charset and current locale (see <code>nl_langinfo()</code> and <code>setlocale()</code>), in this order. Not recommended.     <p><b>Note</b>:  Any other character sets are not recognized. The default encoding will be used instead and a warning will be emitted. </p>
	 * @param bool $double_encode <p>When <code>double_encode</code> is turned off PHP will not encode existing html entities, the default is to convert everything.</p>
	 ...
	 * <p>This function is the opposite of <code>htmlspecialchars()</code>. It converts special HTML entities back to characters.</p><p>The converted entities are: <i>&amp;amp;</i>, <i>&amp;quot;</i> (when <b><code>ENT_NOQUOTES</code></b> is not set), <i>&amp;#039;</i> (when <b><code>ENT_QUOTES</code></b> is set), <i>&amp;lt;</i> and <i>&amp;gt;</i>.</p>
	 ...
	 * @param string $str <p>The input string.</p>
	 * @param string $character_mask <p>You can also specify the characters you want to strip, by means of the <code>character_mask</code> parameter. Simply list all characters that you want to be stripped. With <i>..</i> you can specify a range of characters.</p>
	 * @return string <p>This function returns a string with whitespace stripped from the beginning of <code>str</code>. Without the second parameter, <b>ltrim()</b> will strip these characters:</p><ul> <li>  " " (ASCII <i>32</i> (<i>0x20</i>)), an ordinary space.  </li> <li>  "\t" (ASCII <i>9</i> (<i>0x09</i>)), a tab.  </li> <li>  "\n" (ASCII <i>10</i> (<i>0x0A</i>)), a new line (line feed).  </li> <li>  "\r" (ASCII <i>13</i> (<i>0x0D</i>)), a carriage return.  </li> <li>  "\0" (ASCII <i>0</i> (<i>0x00</i>)), the <i>NUL</i>-byte.  </li> <li>  "\x0B" (ASCII <i>11</i> (<i>0x0B</i>)), a vertical tab.  </li> </ul>
	 ...
	 * @param string $str <p>The input string.</p>
	 * @param int $phonemes <p>This parameter restricts the returned metaphone key to <code>phonemes</code> characters in length. The default value of <i>0</i> means no restriction.</p>
	 * @return string <p>Returns the metaphone key as a string, or <b><code>FALSE</code></b> on failure.</p>
	 ...
	 * @param string $format <p>The format specification consists of the following sequence:</p><ul> <li><p>a <i>%</i> character</p></li> <li><p>optional flags</p></li> <li><p>optional field width</p></li> <li><p>optional left precision</p></li> <li><p>optional right precision</p></li> <li><p>a required conversion character</p></li> </ul> <p></p>Flags <p>One or more of the optional flags below can be used:</p>  <i>=</i>f  <p>The character <i>=</i> followed by a (single byte) character f to be used as the numeric fill character. The default fill character is space.</p>   <i>^</i>  <p>Disable the use of grouping characters (as defined by the current locale).</p>   <i>+</i> or <i>(</i>  <p>Specify the formatting style for positive and negative numbers. If <i>+</i> is used, the locale's equivalent for <i>+</i> and <i>-</i> will be used. If <i>(</i> is used, negative amounts are enclosed in parenthesis. If no specification is given, the default is <i>+</i>.</p>   <i>!</i>  <p>Suppress the currency symbol from the output string.</p>   <i>-</i>  <p>If present, it will make all fields left-justified (padded to the right), as opposed to the default which is for the fields to be right-justified (padded to the left).</p>   <p></p>Field width <p></p>  w  <p>A decimal digit string specifying a minimum field width. Field will be right-justified unless the flag <i>-</i> is used. Default value is 0 (zero).</p>   <p></p>Left precision <p></p>  <i>#</i>n  <p>The maximum number of digits (n) expected to the left of the decimal character (e.g. the decimal point). It is used usually to keep formatted output aligned in the same columns, using the fill character if the number of digits is less than n. If the number of actual digits is bigger than n, then this specification is ignored.</p> <p>If grouping has not been suppressed using the <i>^</i> flag, grouping separators will be inserted before the fill characters (if any) are added. Grouping separators will not be applied to fill characters, even if the fill character is a digit.</p> <p>To ensure alignment, any characters appearing before or after the number in the formatted output such as currency or sign symbols are padded as necessary with space characters to make their positive and negative formats an equal length.</p>   <p></p> Right precision  <p></p>  <i>.</i>p  <p>A period followed by the number of digits (p) after the decimal character. If the value of p is 0 (zero), the decimal character and the digits to its right will be omitted. If no right precision is included, the default will dictated by the current local in use. The amount being formatted is rounded to the specified number of digits prior to formatting.</p>   <p></p> Conversion characters  <p></p>  <i>i</i>  <p>The number is formatted according to the locale's international currency format (e.g. for the USA locale: USD 1,234.56).</p>   <i>n</i>  <p>The number is formatted according to the locale's national currency format (e.g. for the de_DE locale: EU1.234,56).</p>   <i>%</i>  <p>Returns the <i>%</i> character.</p>
	 ...
	 * <p>Returns <code>string</code> with <code>&lt;br /&gt;</code> or <code>&lt;br&gt;</code> inserted before all newlines (<i>\r\n</i>, <i>\n\r</i>, <i>\n</i> and <i>\r</i>).</p>
	 * @param string $string <p>The input string.</p>
	 ...
	 * <p>Returns a version of str with a backslash character (<i>\</i>) before every character that is among these:</p>
	 * @param string $str <p>The input string.</p>
	 ...
	 * @param float $percent <p>By passing a reference as third argument, <b>similar_text()</b> will calculate the similarity in percent, by dividing the result of <b>similar_text()</b> by the average of the lengths of the given strings times <i>100</i>.</p>
	 ...
	 * @param string $str <p>The input string.</p>
	 * @return string <p>Returns the soundex key as a string.</p>
	 ...
	 * @param string $format <p>The format string is composed of zero or more directives: ordinary characters (excluding <i>%</i>) that are copied directly to the result and <i>conversion specifications</i>, each of which results in fetching its own parameter. This applies to both <b>sprintf()</b> and <code>printf()</code>.</p> <p>Each conversion specification consists of a percent sign (<i>%</i>), followed by one or more of these elements, in order:</p><ol> <li>  An optional <i>sign specifier</i> that forces a sign (- or +) to be used on a number. By default, only the - sign is used on a number if it's negative. This specifier forces positive numbers to have the + sign attached as well.  </li> <li>  An optional <i>padding specifier</i> that says what character will be used for padding the results to the right string size. This may be a space character or a <i>0</i> (zero character). The default is to pad with spaces. An alternate padding character can be specified by prefixing it with a single quote (<i>'</i>). See the examples below.  </li> <li>  An optional <i>alignment specifier</i> that says if the result should be left-justified or right-justified. The default is right-justified; a <i>-</i> character here will make it left-justified.  </li> <li>  An optional number, a <i>width specifier</i> that says how many characters (minimum) this conversion should result in.  </li> <li>  An optional <i>precision specifier</i> in the form of a period (<i>.</i>) followed by an optional decimal digit string that says how many decimal digits should be displayed for floating-point numbers. When using this specifier on a string, it acts as a cutoff point, setting a maximum character limit to the string. Additionally, the character to use when padding a number may optionally be specified between the period and the digit.  </li> <li> <p>A <i>type specifier</i> that says what type the argument data should be treated as. Possible types:</p><ul> <li> <i>%</i> - a literal percent character. No argument is required. </li> <li> <i>b</i> - the argument is treated as an integer and presented as a binary number. </li> <li> <i>c</i> - the argument is treated as an integer and presented as the character with that ASCII value. </li> <li> <i>d</i> - the argument is treated as an integer and presented as a (signed) decimal number. </li> <li> <i>e</i> - the argument is treated as scientific notation (e.g. 1.2e+2). The precision specifier stands for the number of digits after the decimal point since PHP 5.2.1. In earlier versions, it was taken as number of significant digits (one less). </li> <li> <i>E</i> - like <i>%e</i> but uses uppercase letter (e.g. 1.2E+2). </li> <li> <i>f</i> - the argument is treated as a float and presented as a floating-point number (locale aware). </li> <li> <i>F</i> - the argument is treated as a float and presented as a floating-point number (non-locale aware). Available since PHP 5.0.3. </li> <li> <i>g</i> - shorter of <i>%e</i> and <i>%f</i>. </li> <li> <i>G</i> - shorter of <i>%E</i> and <i>%f</i>. </li> <li> <i>o</i> - the argument is treated as an integer and presented as an octal number. </li> <li> <i>s</i> - the argument is treated as and presented as a string. </li> <li> <i>u</i> - the argument is treated as an integer and presented as an unsigned decimal number. </li> <li> <i>x</i> - the argument is treated as an integer and presented as a hexadecimal number (with lowercase letters). </li> <li> <i>X</i> - the argument is treated as an integer and presented as a hexadecimal number (with uppercase letters). </li> </ul> </li> </ol> <p>Variables will be co-erced to a suitable type for the specifier:</p> <b>Type Handling</b>   Type Specifiers     <i>string</i> <i>s</i>   <i>integer</i>  <i>d</i>, <i>u</i>, <i>c</i>, <i>o</i>, <i>x</i>, <i>X</i>, <i>b</i>    <i>double</i>  <i>g</i>, <i>G</i>, <i>e</i>, <i>E</i>, <i>f</i>, <i>F</i>     <p><b>Warning</b></p> <p>Attempting to use a combination of the string and width specifiers with character sets that require more than one byte per character may result in unexpected results</p>  <p>The format string supports argument numbering/swapping. Here is an example:</p> <p><b>Example #1 Argument swapping</b></p>  <code> &lt;&#63;php<br>$num&nbsp;=&nbsp;5;<br>$location&nbsp;=&nbsp;'tree';<br><br>$format&nbsp;=&nbsp;'There&nbsp;are&nbsp;%d&nbsp;monkeys&nbsp;in&nbsp;the&nbsp;%s';<br>echo&nbsp;sprintf($format,&nbsp;$num,&nbsp;$location);<br>&#63;&gt;  </code>   This will output "There are 5 monkeys in the tree". But imagine we are creating a format string in a separate file, commonly because we would like to internationalize it and we rewrite it as:  <p><b>Example #2 Argument swapping</b></p>  <code> &lt;&#63;php<br>$format&nbsp;=&nbsp;'The&nbsp;%s&nbsp;contains&nbsp;%d&nbsp;monkeys';<br>echo&nbsp;sprintf($format,&nbsp;$num,&nbsp;$location);<br>&#63;&gt;  </code>   We now have a problem. The order of the placeholders in the format string does not match the order of the arguments in the code. We would like to leave the code as is and simply indicate in the format string which arguments the placeholders refer to. We would write the format string like this instead:  <p><b>Example #3 Argument swapping</b></p>  <code> &lt;&#63;php<br>$format&nbsp;=&nbsp;'The&nbsp;%2$s&nbsp;contains&nbsp;%1$d&nbsp;monkeys';<br>echo&nbsp;sprintf($format,&nbsp;$num,&nbsp;$location);<br>&#63;&gt;  </code>   An added benefit here is that you can repeat the placeholders without adding more arguments in the code. For example:  <p><b>Example #4 Argument swapping</b></p>  <code> &lt;&#63;php<br>$format&nbsp;=&nbsp;'The&nbsp;%2$s&nbsp;contains&nbsp;%1$d&nbsp;monkeys.<br>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;That\'s&nbsp;a&nbsp;nice&nbsp;%2$s&nbsp;full&nbsp;of&nbsp;%1$d&nbsp;monkeys.';<br>echo&nbsp;sprintf($format,&nbsp;$num,&nbsp;$location);<br>&#63;&gt;  </code>   When using argument swapping, the <i>n$</i> <i>position specifier</i> must come immediately after the percent sign (<i>%</i>), before any other specifiers, as shown in the example below.  <p><b>Example #5 Specifying padding character</b></p>  <code> &lt;&#63;php<br>echo&nbsp;sprintf("%'.9d\n",&nbsp;123);<br>echo&nbsp;sprintf("%'.09d\n",&nbsp;123);<br>&#63;&gt;  </code>  <p>The above example will output:</p>  <pre> ......123 000000123 </pre>    <p><b>Example #6 Position specifier with other specifiers</b></p>  <code> &lt;&#63;php<br>$format&nbsp;=&nbsp;'The&nbsp;%2$s&nbsp;contains&nbsp;%1$04d&nbsp;monkeys';<br>echo&nbsp;sprintf($format,&nbsp;$num,&nbsp;$location);<br>&#63;&gt;  </code>  <p>The above example will output:</p>  <pre> The tree contains 0005 monkeys </pre>   <p><b>Note</b>:</p><p>Attempting to use a position specifier greater than <b><code>PHP_INT_MAX</code></b> will result in <b>sprintf()</b> generating warnings.</p>  <p><b>Warning</b></p> <p>The <i>c</i> type specifier ignores padding and width</p>
	 * @param mixed $args
	 * @param mixed $_
	 * @return string <p>Returns a string produced according to the formatting string <code>format</code>, or <b><code>FALSE</code></b> on failure.</p>
	 ...
	 * @param string $format <p>The interpreted format for <code>str</code>, which is described in the documentation for <code>sprintf()</code> with following differences:</p><ul> <li> Function is not locale-aware. </li> <li> <i>F</i>, <i>g</i>, <i>G</i> and <i>b</i> are not supported. </li> <li> <i>D</i> stands for decimal number. </li> <li> <i>i</i> stands for integer with base detection. </li> <li> <i>n</i> stands for number of characters processed so far. </li> <li> <i>s</i> stops reading at any whitespace character. </li> </ul>
	 * @param mixed $_ <p>Optionally pass in variables by reference that will contain the parsed values.</p>
	 * @return mixed <p>If only two parameters were passed to this function, the values parsed will be returned as an array. Otherwise, if optional parameters are passed, the function will return the number of assigned values. The optional parameters must be passed by reference.</p><p>If there are more substrings expected in the <code>format</code> than there are available within <code>str</code>, <i>-1</i> will be returned.</p>
	 ...
	 * <p>Parses a string input for fields in CSV format and returns an array containing the fields read.</p><p><b>Note</b>:</p><p>The locale settings are taken into account by this function. If <i>LC_CTYPE</i> is e.g. <i>en_US.UTF-8</i>, strings in one-byte encodings may be read wrongly by this function.</p>
	 ...
	 * <p>This function returns a string or an array with all occurrences of <code>search</code> in <code>subject</code> replaced with the given <code>replace</code> value.</p><p>If you don't need fancy replacing rules (like regular expressions), you should always use this function instead of <code>preg_replace()</code>.</p>
	 * @param mixed $search <p>The value being searched for, otherwise known as the <i>needle</i>. An array may be used to designate multiple needles.</p>
	 * @param mixed $replace <p>The replacement value that replaces found <code>search</code> values. An array may be used to designate multiple replacements.</p>
	 * @param mixed $subject <p>The string or array being searched and replaced on, otherwise known as the <i>haystack</i>.</p> <p>If <code>subject</code> is an array, then the search and replace is performed with every entry of <code>subject</code>, and the return value is an array as well.</p>
	 * @param int $count <p>If passed, this will be set to the number of replacements performed.</p>
	 ...
	 * @param mixed $needle <p>If <code>needle</code> is not a string, it is converted to an integer and applied as the ordinal value of a character.</p>
	 ...
	 * <p>Returns the length of the initial segment of <code>subject</code> which does <i>not</i> contain any of the characters in <code>mask</code>.</p><p>If <code>start</code> and <code>length</code> are omitted, then all of <code>subject</code> will be examined. If they are included, then the effect will be the same as calling <i>strcspn(substr($subject, $start, $length), $mask)</i> (see substr for more information).</p>
	 * @param string $subject <p>The string to examine.</p>
	 * @param string $mask <p>The string containing every disallowed character.</p>
	 * @param int $start <p>The position in <code>subject</code> to start searching.</p> <p>If <code>start</code> is given and is non-negative, then <b>strcspn()</b> will begin examining <code>subject</code> at the <code>start</code>'th position. For instance, in the string '<i>abcdef</i>', the character at position <i>0</i> is '<i>a</i>', the character at position <i>2</i> is '<i>c</i>', and so forth.</p> <p>If <code>start</code> is given and is negative, then <b>strcspn()</b> will begin examining <code>subject</code> at the <code>start</code>'th position from the end of <code>subject</code>.</p>
	 ...
	 * @param string $str <p>The input string.</p>
	 * @param string $allowable_tags <p>You can use the optional second parameter to specify tags which should not be stripped.</p> <p><b>Note</b>:</p><p>HTML comments and PHP tags are also stripped. This is hardcoded and can not be changed with <code>allowable_tags</code>.</p>  <p><b>Note</b>:</p><p>In PHP 5.3.4 and later, self-closing XHTML tags are ignored and only non-self-closing tags should be used in <code>allowable_tags</code>. For example, to allow both <i>&lt;br&gt;</i> and <i>&lt;br/&gt;</i>, you should use:</p>   <code> &lt;&#63;php<br>strip_tags($input,&nbsp;'&lt;br&gt;');<br>&#63;&gt;  </code>
	 ...
	 * <p>Returns a string with backslashes stripped off. Recognizes C-like <i>\n</i>, <i>\r</i> ..., octal and hexadecimal representation.</p>
	 ...
	 * @param string $needle <p>Note that the <code>needle</code> may be a string of one or more characters.</p> <p>If <code>needle</code> is not a string, it is converted to an integer and applied as the ordinal value of a character.</p>
	 ...
	 * <p>Un-quotes a quoted string.</p><p><b>Note</b>:</p><p>If magic_quotes_sybase is on, no backslashes are stripped off but two apostrophes are replaced by one instead.</p><p>An example use of <b>stripslashes()</b> is when the PHP directive magic_quotes_gpc is <i>on</i> (it was on by default before PHP 5.4), and you aren't inserting this data into a place (such as a database) that requires escaping. For example, if you're simply outputting data straight from an HTML form.</p>
	 * @param string $str <p>The input string.</p>
	 * @return string <p>Returns a string with backslashes stripped off. (<i>\'</i> becomes <i>'</i> and so on.) Double backslashes (<i>\\</i>) are made into a single backslash (<i>\</i>).</p>
	 ...
	 * @return int <p>The length of the <code>string</code> on success, and <i>0</i> if the <code>string</code> is empty.</p>
	 ...
	 * @param string $haystack <p>The string to search in</p>
	 * @param mixed $needle <p>If <code>needle</code> contains more than one character, only the first is used. This behavior is different from that of <code>strstr()</code>.</p> <p>If <code>needle</code> is not a string, it is converted to an integer and applied as the ordinal value of a character.</p>
	 ...
	 * @param string $haystack <p>The string to search in.</p>
	 * @param string $needle <p>If <code>needle</code> is not a string, it is converted to an integer and applied as the ordinal value of a character.</p>
	 * @param int $offset <p>If specified, search will start this number of characters counted from the beginning of the string. If the value is negative, search will instead start from that many characters from the end of the string, searching backwards.</p>
	 * @return int <p>Returns the position where the needle exists relative to the beginnning of the <code>haystack</code> string (independent of search direction or offset). Also note that string positions start at 0, and not 1.</p><p>Returns <b><code>FALSE</code></b> if the needle was not found.</p><p><b>Warning</b></p><p>This function may return Boolean <b><code>FALSE</code></b>, but may also return a non-Boolean value which evaluates to <b><code>FALSE</code></b>. Please read the section on Booleans for more information. Use the === operator for testing the return value of this function.</p>
	 ...
	 * <p>Finds the length of the initial segment of <code>subject</code> that contains <i>only</i> characters from <code>mask</code>.</p><p>If <code>start</code> and <code>length</code> are omitted, then all of <code>subject</code> will be examined. If they are included, then the effect will be the same as calling <i>strspn(substr($subject, $start, $length), $mask)</i> (see substr for more information).</p><p>The line of code:</p>
	 * @param string $subject <p>The string to examine.</p>
	 * @param string $mask <p>The list of allowable characters.</p>
	 * @param int $start <p>The position in <code>subject</code> to start searching.</p> <p>If <code>start</code> is given and is non-negative, then <b>strspn()</b> will begin examining <code>subject</code> at the <code>start</code>'th position. For instance, in the string '<i>abcdef</i>', the character at position <i>0</i> is '<i>a</i>', the character at position <i>2</i> is '<i>c</i>', and so forth.</p> <p>If <code>start</code> is given and is negative, then <b>strspn()</b> will begin examining <code>subject</code> at the <code>start</code>'th position from the end of <code>subject</code>.</p>
	 * @param int $length <p>The length of the segment from <code>subject</code> to examine.</p> <p>If <code>length</code> is given and is non-negative, then <code>subject</code> will be examined for <code>length</code> characters after the starting position.</p> <p>If <code>length</code> is given and is negative, then <code>subject</code> will be examined from the starting position up to <code>length</code> characters from the end of <code>subject</code>.</p>
	 ...
	 * <p>If given three arguments, this function returns a copy of <code>str</code> where all occurrences of each (single-byte) character in <code>from</code> have been translated to the corresponding character in <code>to</code>, i.e., every occurrence of <i>$from[$n]</i> has been replaced with <i>$to[$n]</i>, where <i>$n</i> is a valid offset in both arguments.</p><p>If <code>from</code> and <code>to</code> have different lengths, the extra characters in the longer of the two are ignored. The length of <code>str</code> will be the same as the return value's.</p><p>If given two arguments, the second should be an <code>array</code> in the form <i>array('from' =&gt; 'to', ...)</i>. The return value is a <code>string</code> where all the occurrences of the array keys have been replaced by the corresponding values. The longest keys will be tried first. Once a substring has been replaced, its new value will not be searched again.</p><p>In this case, the keys and the values may have any length, provided that there is no empty key; additionally, the length of the return value may differ from that of <code>str</code>. However, this function will be the most efficient when all the keys have the same size.</p>
	 ...
	 * @return string <p>Returns the translated <code>string</code>.</p><p>If <code>replace_pairs</code> contains a key which is an empty <code>string</code> (<i>""</i>), <b><code>FALSE</code></b> will be returned. If the <code>str</code> is not a scalar then it is not typecasted into a string, instead a warning is raised and <b><code>NULL</code></b> is returned.</p>
	 ...
	 * @param int $length <p>The length of the comparison. The default value is the largest of the length of the <code>str</code> compared to the length of <code>main_str</code> less the <code>offset</code>.</p>
	 * @param bool $case_insensitivity <p>If <code>case_insensitivity</code> is <b><code>TRUE</code></b>, comparison is case insensitive.</p>
	 * @return int <p>Returns &lt; 0 if <code>main_str</code> from position <code>offset</code> is less than <code>str</code>, &gt; 0 if it is greater than <code>str</code>, and 0 if they are equal. If <code>offset</code> is equal to or greater than the length of <code>main_str</code>, or the <code>length</code> is set and is less than 1 (prior to PHP 5.5.11), <b>substr_compare()</b> prints a warning and returns <b><code>FALSE</code></b>.</p>
	 ...
	 * @return int <p>This function returns an <code>integer</code>.</p>
	 ...
	 * @param string $character_mask <p>Optionally, the stripped characters can also be specified using the <code>character_mask</code> parameter. Simply list all characters that you want to be stripped. With <i>..</i> you can specify a range of characters.</p>
	 ...
	 * @param string $format <p>See <code>sprintf()</code> for a description of <code>format</code>.</p>
	 * @param array $args
	 * @return string <p>Return array values as a formatted string according to <code>format</code> (which is described in the documentation for <code>sprintf()</code>).</p>
