...
namespace {
	 ...
	 * <p>This function converts the string <code>data</code> from the <i>UTF-8</i> encoding to <i>ISO-8859-1</i>. Bytes in the string which are not valid <i>UTF-8</i>, and <i>UTF-8</i> characters which do not exist in <i>ISO-8859-1</i> (that is, characters above <i>U+00FF</i>) are replaced with <i>&#63;</i>.</p><p><b>Note</b>:</p><p>Many web pages marked as using the <i>ISO-8859-1</i> character encoding actually use the similar <i>Windows-1252</i> encoding, and web browsers will interpret <i>ISO-8859-1</i> web pages as <i>Windows-1252</i>. <i>Windows-1252</i> features additional printable characters, such as the Euro sign (<i>&euro;</i>) and curly quotes (<i>&ldquo;</i> <i>&rdquo;</i>), instead of certain <i>ISO-8859-1</i> control characters. This function will not convert such <i>Windows-1252</i> characters correctly. Use a different function if <i>Windows-1252</i> conversion is required.</p>
	 * @param string $data <p>A UTF-8 encoded string.</p>
	 * @return string <p>Returns the ISO-8859-1 translation of <code>data</code>.</p>
	 ...
	 * <p>This function converts the string <code>data</code> from the <i>ISO-8859-1</i> encoding to <i>UTF-8</i>.</p><p><b>Note</b>:</p><p>Many web pages marked as using the <i>ISO-8859-1</i> character encoding actually use the similar <i>Windows-1252</i> encoding, and web browsers will interpret <i>ISO-8859-1</i> web pages as <i>Windows-1252</i>. <i>Windows-1252</i> features additional printable characters, such as the Euro sign (<i>&euro;</i>) and curly quotes (<i>&ldquo;</i> <i>&rdquo;</i>), instead of certain <i>ISO-8859-1</i> control characters. This function will not convert such <i>Windows-1252</i> characters correctly. Use a different function if <i>Windows-1252</i> conversion is required.</p>
	 * @param string $data <p>An ISO-8859-1 string.</p>
	 * @return string <p>Returns the UTF-8 translation of <code>data</code>.</p>
	 ...
	 * <p>Gets the XML parser error string associated with the given <code>code</code>.</p>
	 * @param int $code <p>An error code from <code>xml_get_error_code()</code>.</p>
	 * @return string <p>Returns a string with a textual description of the error <code>code</code>, or <b><code>FALSE</code></b> if no description was found.</p>
	 ...
	 * @param resource $parser <p>A reference to the XML parser to get byte index from.</p>
	 ...
	 * @param resource $parser <p>A reference to the XML parser to get column number from.</p>
	 ...
	 * @param resource $parser <p>A reference to the XML parser to get line number from.</p>
	 ...
	 * @param resource $parser <p>A reference to the XML parser to get error code from.</p>
	 ...
	 * @param resource $parser <p>A reference to the XML parser to use.</p>
	 ...
	 * @param resource $parser <p>A reference to the XML parser.</p>
	 ...
	 * <p><b>xml_parser_create()</b> creates a new XML parser and returns a resource handle referencing it to be used by the other XML functions.</p>
	 * @param string $encoding <p>The optional <code>encoding</code> specifies the character encoding for the input/output in PHP 4. Starting from PHP 5, the input encoding is automatically detected, so that the <code>encoding</code> parameter specifies only the output encoding. In PHP 4, the default output encoding is the same as the input charset. If empty string is passed, the parser attempts to identify which encoding the document is encoded in by looking at the heading 3 or 4 bytes. In PHP 5.0.0 and 5.0.1, the default output charset is ISO-8859-1, while in PHP 5.0.2 and upper is UTF-8. The supported encodings are <i>ISO-8859-1</i>, <i>UTF-8</i> and <i>US-ASCII</i>.</p>
	 * @return resource <p>Returns a resource handle for the new XML parser.</p>
	 ...
	 * <p><b>xml_parser_create_ns()</b> creates a new XML parser with XML namespace support and returns a resource handle referencing it to be used by the other XML functions.</p>
	 * @param string $encoding <p>The input encoding is automatically detected, so that the <code>encoding</code> parameter specifies only the output encoding. In PHP 5.0.0 and 5.0.1, the default output charset is ISO-8859-1, while in PHP 5.0.2 and upper is UTF-8. The supported encodings are <i>ISO-8859-1</i>, <i>UTF-8</i> and <i>US-ASCII</i>.</p>
	...
	/**
	 * Free an XML parser
	 * <p>Frees the given XML <code>parser</code>.</p>
	 * @param resource $parser A reference to the XML parser to free.
	 * @return bool <p>This function returns <b><code>FALSE</code></b> if <code>parser</code> does not refer to a valid parser, or else it frees the parser and returns <b><code>TRUE</code></b>.</p>
	 ...
	 * @param int $option Which option to fetch. <b><code>XML_OPTION_CASE_FOLDING</code></b> and <b><code>XML_OPTION_TARGET_ENCODING</code></b> are available. See <code>xml_parser_set_option()</code> for their description.
	 * @return mixed <p>This function returns <b><code>FALSE</code></b> if <code>parser</code> does not refer to a valid parser or if <code>option</code> isn't valid (generates also a <b><code>E_WARNING</code></b>). Else the option's value is returned.</p>
	 ...
	 * @param resource $parser <p>A reference to the XML parser to set up default handler function.</p>
	 ...
	 * <p>This function allows to use <code>parser</code> inside <code>object</code>. All callback functions could be set with <code>xml_set_element_handler()</code> etc and assumed to be methods of <code>object</code>.</p><p>In addition to calling <code>xml_parser_free()</code> when the parsing is finished, as of PHP 7.0.0 it is necessary to also explicitly unset the reference to <code>parser</code> to avoid memory leaks.</p>
	 * @param resource $parser <p>A reference to the XML parser to use inside the object.</p>
	 ...
	 * @param resource $parser <p>A reference to the XML parser to set up processing instruction (PI) handler function.</p>
	 * @param callable $handler <p><code>handler</code> is a string containing the name of a function that must exist when <code>xml_parse()</code> is called for <code>parser</code>.</p> <p>The function named by <code>handler</code> must accept three parameters:</p> handler ( resource <code>$parser</code> , string <code>$target</code> , string <code>$data</code> )   <code>parser</code>   The first parameter, parser, is a reference to the XML parser calling the handler.    <code>target</code>   The second parameter, <code>target</code>, contains the PI target.    <code>data</code>   The third parameter, <code>data</code>, contains the PI data.    <p>If a handler function is set to an empty string, or <b><code>FALSE</code></b>, the handler in question is disabled.</p> <p><b>Note</b>: Instead of a function name, an array containing an object reference and a method name can also be supplied.</p>
	 * @return bool <p>Returns <b><code>TRUE</code></b> on success or <b><code>FALSE</code></b> on failure.</p>
	...
	/**
	 * Holds the SAX implementation method. Can be <i>libxml</i> or <i>expat</i>.
	 */
