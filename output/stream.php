...
namespace {
	...
	class php_user_filter {
		...
		/**
		 * @var string <p>Name of the filter registered by <code>stream_filter_append()</code>.</p>
		...
		/**
		 * @var ?resource
		 * @link https://php.net/manual/en/class.php-user-filter.php#php-user-filter.props.stream
		 */
		public $stream = null;
	...
	class streamWrapper {
		 ...
		 * @return string|false <p>If there are less than <code>count</code> bytes available, as many as are available should be returned. If no more data is available, an empty string should be returned. To signal that reading failed, <b><code>false</code></b> should be returned.</p>
		 ...
		 * @return array|false <p>See <code>stat()</code>.</p>
		 ...
		 * @return array|false <p>Should return an <code>array</code> with the same elements as <code>stat()</code> does. Unknown or unavailable values should be set to a rational value (usually <b><code>0</code></b>). Special attention should be payed to <code>mode</code> as documented under <code>stat()</code>. Should return <b><code>false</code></b> on failure.</p>
	 ...
	 * <p>This function is called whenever there is the need to access and operate on the content contains in a brigade. It is typically called from <code>php_user_filter::filter()</code>.</p>
	 * @param resource $brigade <p>The brigade to return a bucket object from.</p>
	 * @return ?object <p>Returns a bucket object with the properties listed below or <b><code>null</code></b>.</p>  data (<code>string</code>)   <p><code>data</code> <code>bucket</code> The current string in the bucket.</p>   datalen (<code>integer</code>)   <p><code>datalen</code> <code>bucket</code> The length of the string in the bucket.</p>
	 * @link https://php.net/manual/en/function.stream-bucket-make-writeable.php
	 * @see stream_bucket_append(), stream_bucket_prepend()
	 * @since PHP 5, PHP 7, PHP 8
	 */
	 ...
	 * @param ?array $options <p>Must be an associative array of associative arrays in the format <code>$arr['wrapper']['option'] = $value</code>, or <b><code>null</code></b>. Refer to context options for a list of available wrappers and options.</p> <p>Defaults to <b><code>null</code></b>.</p>
	 * @param ?array $params <p>Must be an associative array in the format <code>$arr['parameter'] = $value</code>, or <b><code>null</code></b>. Refer to context parameters for a listing of standard stream parameters.</p>
	 ...
	 * @param ?array $options <code>options</code> must be an associative array of associative arrays in the format <code>$arr['wrapper']['option'] = $value</code>, or <b><code>null</code></b>.
	 * @return resource <p>A stream context <code>resource</code>.</p>
	 * @link https://php.net/manual/en/function.stream-context-get-default.php
	 * @see stream_context_create(), stream_context_set_default()
	 ...
	 * @param resource $context <p>A stream <code>resource</code> or a context <code>resource</code></p>
	 ...
	 * @param string $wrapper <p>The name of the wrapper (which may be different than the protocol). Refer to context options and parameters for a listing of stream options.</p>
	 * @param string $option <p>The name of the option.</p>
	 * @param mixed $value <p>The value of the option.</p>
	 * @return bool <p>Returns <b><code>true</code></b> on success or <b><code>false</code></b> on failure.</p>
	 ...
	 * @param array $params <p>An associative array of parameters to be set in the following format: <code>$params['paramname'] = "paramvalue";</code>.</p>  <b>Supported parameters</b>   Parameter Purpose      <code>notification</code>   Name of user-defined callback function to be called whenever a stream triggers a notification. Only supported for http:// and ftp:// stream wrappers.     <code>options</code>   Array of options as in context options and parameters.
	 * @return bool <p>Returns <b><code>true</code></b> on success or <b><code>false</code></b> on failure.</p>
	 ...
	 * <p>Makes a copy of up to <code>length</code> bytes of data from the current position (or from the <code>offset</code> position, if specified) in <code>from</code> to <code>to</code>. If <code>length</code> is <b><code>null</code></b>, all remaining content in <code>from</code> will be copied.</p>
	 * @param resource $from <p>The source stream</p>
	 * @param resource $to <p>The destination stream</p>
	 * @param ?int $length <p>Maximum bytes to copy. By default all bytes left are copied.</p>
	 ...
	 * @param string $class <p>To implement a filter, you need to define a class as an extension of <code>php_user_filter</code> with a number of member functions. When performing read/write operations on the stream to which your filter is attached, PHP will pass the data through your filter (and any other filters attached to that stream) so that the data may be modified as desired. You must implement the methods exactly as described in <code>php_user_filter</code> - doing otherwise will lead to undefined behaviour.</p>
	 ...
	 * <p>Identical to <code>file_get_contents()</code>, except that <b>stream_get_contents()</b> operates on an already open stream resource and returns the remaining contents in a string, up to <code>length</code> bytes and starting at the specified <code>offset</code>.</p>
	 * @param resource $stream <p>A stream resource (e.g. returned from <code>fopen()</code>)</p>
	 * @param ?int $length <p>The maximum bytes to read. Defaults to <b><code>null</code></b> (read all the remaining buffer).</p>
	 ...
	 * <p>Gets a line from the given handle.</p><p>Reading ends when <code>length</code> bytes have been read, when the non-empty string specified by <code>ending</code> is found (which is <i>not</i> included in the return value), or on EOF (whichever comes first).</p><p>This function is nearly identical to <code>fgets()</code> except in that it allows end of line delimiters other than the standard \n, \r, and \r\n, and does <i>not</i> return the delimiter itself.</p>
	 * @param resource $stream <p>A valid file handle.</p>
	 * @param int $length <p>The maximum number of bytes to read from the handle. Negative values are not supported. Zero (<code>0</code>) means the default socket chunk size, i.e. <code>8192</code> bytes.</p>
	 * @param string $ending <p>An optional string delimiter.</p>
	 * @return string|false <p>Returns a string of up to <code>length</code> bytes read from the file pointed to by <code>stream</code>, or <b><code>false</code></b> on failure.</p>
	 ...
	 * @param string $protocol <p>The wrapper name to be registered. Valid protocol names must contain alphanumerics, dots (.), plusses (+), or hyphens (-) only.</p>
	 ...
	 * @param ?int $seconds <p>The <code>seconds</code> and <code>microseconds</code> together form the <i>timeout</i> parameter, <code>seconds</code> specifies the number of seconds while <code>microseconds</code> the number of microseconds. The <code>timeout</code> is an upper bound on the amount of time that <b>stream_select()</b> will wait before it returns. If <code>seconds</code> and <code>microseconds</code> are both set to <code>0</code>, <b>stream_select()</b> will not wait for data - instead it will return immediately, indicating the current status of the streams.</p> <p>If <code>seconds</code> is <b><code>null</code></b> <b>stream_select()</b> can block indefinitely, returning only when an event on one of the watched streams occurs (or if a signal interrupts the system call).</p> <p><b>Warning</b></p> <p>Using a timeout value of <code>0</code> allows you to instantaneously poll the status of the streams, however, it is NOT a good idea to use a <code>0</code> timeout value in a loop as it will cause your script to consume too much CPU time.</p> <p>It is much better to specify a timeout value of a few seconds, although if you need to be checking and running other code concurrently, using a timeout value of at least <code>200000</code> microseconds will help reduce the CPU usage of your script.</p> <p>Remember that the timeout value is the maximum time that will elapse; <b>stream_select()</b> will return as soon as the requested streams are ready for use.</p>
	 * @param ?int $microseconds <p>See <code>seconds</code> description.</p>
	 * @return int|false <p>On success <b>stream_select()</b> returns the number of stream resources contained in the modified arrays, which may be zero if the timeout expires before anything interesting happens. On error <b><code>false</code></b> is returned and a warning raised (this can happen if the system call is interrupted by an incoming signal).</p>
	 ...
	 * <p>Sets the buffering for write operations on the given <code>stream</code> to <code>size</code> bytes.</p>
	 * @param resource $stream <p>The file pointer.</p>
	 ...
	 * @param resource $socket <p>The server socket to accept a connection from.</p>
	 * @param ?float $timeout <p>Override the default socket accept timeout. Time should be given in seconds. By default, default_socket_timeout is used.</p>
	 * @param string $peer_name <p>Will be set to the name (address) of the client which connected, if included and available from the selected transport.</p> <p><b>Note</b>:</p><p>Can also be determined later using <code>stream_socket_get_name()</code>.</p>
	 ...
	 * @return resource|false <p>On success a stream resource is returned which may be used together with the other file functions (such as <code>fgets()</code>, <code>fgetss()</code>, <code>fwrite()</code>, <code>fclose()</code>, and <code>feof()</code>), <b><code>false</code></b> on failure.</p>
	 ...
	 * @since PHP 5, PHP 7, PHP 8
	 */
	function stream_socket_client(string $address, int &$error_code = null, string &$error_message = null, ?float $timeout = null, int $flags = STREAM_CLIENT_CONNECT, ?resource $context = null) {}
	 ...
	 * @param ?int $crypto_method <p>Setup encryption on the stream. Valid methods are</p><ul> <li><b><code>STREAM_CRYPTO_METHOD_SSLv2_CLIENT</code></b></li> <li><b><code>STREAM_CRYPTO_METHOD_SSLv3_CLIENT</code></b></li> <li><b><code>STREAM_CRYPTO_METHOD_SSLv23_CLIENT</code></b></li> <li><b><code>STREAM_CRYPTO_METHOD_ANY_CLIENT</code></b></li> <li><b><code>STREAM_CRYPTO_METHOD_TLS_CLIENT</code></b></li> <li><b><code>STREAM_CRYPTO_METHOD_TLSv1_0_CLIENT</code></b></li> <li><b><code>STREAM_CRYPTO_METHOD_TLSv1_1_CLIENT</code></b></li> <li><b><code>STREAM_CRYPTO_METHOD_TLSv1_2_CLIENT</code></b></li> <li><b><code>STREAM_CRYPTO_METHOD_TLSv1_3_CLIENT</code></b> (as of PHP 7.4.0)</li> <li><b><code>STREAM_CRYPTO_METHOD_SSLv2_SERVER</code></b></li> <li><b><code>STREAM_CRYPTO_METHOD_SSLv3_SERVER</code></b></li> <li><b><code>STREAM_CRYPTO_METHOD_SSLv23_SERVER</code></b></li> <li><b><code>STREAM_CRYPTO_METHOD_ANY_SERVER</code></b></li> <li><b><code>STREAM_CRYPTO_METHOD_TLS_SERVER</code></b></li> <li><b><code>STREAM_CRYPTO_METHOD_TLSv1_0_SERVER</code></b></li> <li><b><code>STREAM_CRYPTO_METHOD_TLSv1_1_SERVER</code></b></li> <li><b><code>STREAM_CRYPTO_METHOD_TLSv1_2_SERVER</code></b></li> <li><b><code>STREAM_CRYPTO_METHOD_TLSv1_3_SERVER</code></b> (as of PHP 7.4.0)</li> </ul> <p>If omitted, the <code>crypto_method</code> context option on the stream's SSL context will be used instead.</p>
	 * @param ?resource $session_stream <p>Seed the stream with settings from <code>session_stream</code>.</p>
	 * @return int|bool <p>Returns <b><code>true</code></b> on success, <b><code>false</code></b> if negotiation has failed or <code>0</code> if there isn't enough data and you should try again (only for non-blocking sockets).</p>
	 * @link https://php.net/manual/en/function.stream-socket-enable-crypto.php
	 * @since PHP 5 >= 5.1.0, PHP 7, PHP 8
	 */
	function stream_socket_enable_crypto($stream, bool $enable, ?int $crypto_method = null, ?resource $session_stream = null): int|bool {}

	/**
	 * Retrieve the name of the local or remote sockets
	 * <p>Returns the local or remote name of a given socket connection.</p>
	 * @param resource $socket <p>The socket to get the name of.</p>
	 * @param bool $remote <p>If set to <b><code>true</code></b> the <code>remote</code> socket name will be returned, if set to <b><code>false</code></b> the <code>local</code> socket name will be returned.</p>
	 * @return string|false <p>The name of the socket, or <b><code>false</code></b> on failure.</p>
	 ...
	 * @return string|false <p>Returns the read data, as a string, or <b><code>false</code></b> on failure.</p>
	 ...
	 * @since PHP 5, PHP 7, PHP 8
	 */
	function stream_socket_recvfrom($socket, int $length, int $flags = 0, ?string &$address = null): string|false {}
	 ...
	 * @return int|false <p>Returns a result code, as an integer, or <b><code>false</code></b> on failure.</p>
	 ...
	 * @return resource|false <p>Returns the created stream, or <b><code>false</code></b> on error.</p>
	 ...
	 * @since PHP 5, PHP 7, PHP 8
	 */
	function stream_socket_server(string $address, int &$error_code = null, string &$error_message = null, int $flags = STREAM_SERVER_BIND | STREAM_SERVER_LISTEN, ?resource $context = null) {}
